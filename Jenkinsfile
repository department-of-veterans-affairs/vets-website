      ...
      try {
        parallel (
          e2e: {
            ...
            sh "docker build -t vets-website-puppeteer:${commonStages.IMAGE_TAG} --file Dockerfile.puppeteer . && docker-compose -p e2e run --rm --entrypoint=xvfb-run --server-args=\"-screen 0 1024x768x24\" npm -e BABEL_ENV=test -e BUILDTYPE=vagovprod vets-website-puppeteer --no-color run test:puppeteer:docker"
          },
