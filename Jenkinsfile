      ...
      try {
        if (commonStages.IS_PROD_BRANCH && commonStages.VAGOV_BUILDTYPES.contains('vagovprod')) {
          ...
          parallel (
            ...
            'cypress-1': {
              sh "export IMAGE_TAG=${commonStages.IMAGE_TAG} && docker-compose -p cypress-${env.EXECUTOR_NUMBER} up -d && docker-compose -p cypress-${env.EXECUTOR_NUMBER} run --rm --entrypoint=npm -e CI=true -e NO_COLOR=1 -e STEP=0 -e NUM_STEPS=7 -e PERCY_PARALLEL_NONCE=1234 -e PERCY_TOKEN=$(aws ssm get-parameters --region us-gov-west-1 --names /dsva-vagov/vets-website/common/percy_token_vets-website --query Parameters[0].Value --with-decryption | sed \'s/\"//g\') vets-website --no-color run cy:test:docker"
            },     
            'cypress-2': {
              sh "export IMAGE_TAG=${commonStages.IMAGE_TAG} && docker-compose -p cypress2-${env.EXECUTOR_NUMBER} up -d && docker-compose -p cypress2-${env.EXECUTOR_NUMBER} run --rm --entrypoint=npm -e CI=true -e NO_COLOR=1 -e STEP=1 -e NUM_STEPS=7 -e PERCY_PARALLEL_NONCE=1234 -e PERCY_TOKEN=$(aws ssm get-parameters --region us-gov-west-1 --names /dsva-vagov/vets-website/common/percy_token_vets-website --query Parameters[0].Value --with-decryption | sed \'s/\"//g\') vets-website --no-color run cy:test:docker"
            },
            'cypress-3': {
              sh "export IMAGE_TAG=${commonStages.IMAGE_TAG} && docker-compose -p cypress3-${env.EXECUTOR_NUMBER} up -d && docker-compose -p cypress3-${env.EXECUTOR_NUMBER} run --rm --entrypoint=npm -e CI=true -e NO_COLOR=1 -e STEP=2 -e NUM_STEPS=7 -e PERCY_PARALLEL_NONCE=1234 -e PERCY_TOKEN=$(aws ssm get-parameters --region us-gov-west-1 --names /dsva-vagov/vets-website/common/percy_token_vets-website --query Parameters[0].Value --with-decryption | sed \'s/\"//g\') vets-website --no-color run cy:test:docker"
            },
            'cypress-4': {
              sh "export IMAGE_TAG=${commonStages.IMAGE_TAG} && docker-compose -p cypress4-${env.EXECUTOR_NUMBER} up -d && docker-compose -p cypress4-${env.EXECUTOR_NUMBER} run --rm --entrypoint=npm -e CI=true -e NO_COLOR=1 -e STEP=3 -e NUM_STEPS=7 -e PERCY_PARALLEL_NONCE=1234 -e PERCY_TOKEN=$(aws ssm get-parameters --region us-gov-west-1 --names /dsva-vagov/vets-website/common/percy_token_vets-website --query Parameters[0].Value --with-decryption | sed \'s/\"//g\') vets-website --no-color run cy:test:docker"
            },
            'cypress-5': {
              sh "export IMAGE_TAG=${commonStages.IMAGE_TAG} && docker-compose -p cypress5-${env.EXECUTOR_NUMBER} up -d && docker-compose -p cypress5-${env.EXECUTOR_NUMBER} run --rm --entrypoint=npm -e CI=true -e NO_COLOR=1 -e STEP=4 -e NUM_STEPS=7 -e PERCY_PARALLEL_NONCE=1234 -e PERCY_TOKEN=$(aws ssm get-parameters --region us-gov-west-1 --names /dsva-vagov/vets-website/common/percy_token_vets-website --query Parameters[0].Value --with-decryption | sed \'s/\"//g\') vets-website --no-color run cy:test:docker"
            },
            'cypress-6': {
              sh "export IMAGE_TAG=${commonStages.IMAGE_TAG} && docker-compose -p cypress6-${env.EXECUTOR_NUMBER} up -d && docker-compose -p cypress6-${env.EXECUTOR_NUMBER} run --rm --entrypoint=npm -e CI=true -e NO_COLOR=1 -e STEP=5 -e NUM_STEPS=7 -e PERCY_PARALLEL_NONCE=1234 -e PERCY_TOKEN=$(aws ssm get-parameters --region us-gov-west-1 --names /dsva-vagov/vets-website/common/percy_token_vets-website --query Parameters[0].Value --with-decryption | sed \'s/\"//g\') vets-website --no-color run cy:test:docker"
            },
            'cypress-7': {
              sh "export IMAGE_TAG=${commonStages.IMAGE_TAG} && docker-compose -p cypress7-${env.EXECUTOR_NUMBER} up -d && docker-compose -p cypress7-${env.EXECUTOR_NUMBER} run --rm --entrypoint=npm -e CI=true -e NO_COLOR=1 -e STEP=6 -e NUM_STEPS=7 -e PERCY_PARALLEL_NONCE=1234 -e PERCY_TOKEN=$(aws ssm get-parameters --region us-gov-west-1 --names /dsva-vagov/vets-website/common/percy_token_vets-website --query Parameters[0].Value --with-decryption | sed \'s/\"//g\') vets-website --no-color run cy:test:docker"
            }
          )
        }
      } catch (error) {
