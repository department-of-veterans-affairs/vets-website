      ...
      try {
        parallel (
          e2e: {
            ...
            sh "docker-compose -p e2e run --rm --entrypoint='/usr/bin/xvfb-run --server-args=\"-screen 0 1024x768x24\" npm' -e BABEL_ENV=test -e BUILDTYPE=vagovprod vets-website-puppeteer --no-color run test:puppeteer:docker"
          },
