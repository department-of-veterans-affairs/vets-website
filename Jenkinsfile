    ...
    dir("vets-website") {
      try {
        if (commonStages.IS_PROD_BRANCH && commonStages.VAGOV_BUILDTYPES.contains('vagovprod')) {
          ...
          nonce = Date.now();
          steps = 7;
          PERCY_TOKEN = $(aws ssm get-parameters --region us-gov-west-1 --names /dsva-vagov/vets-website/common/percy_token_vets-website --query Parameters[0].Value --with-decryption | sed \'s/\"//g\')
