  ...
  if (IS_BRANCH_FOR_CREATING_CROSS_DOMAIN_REDIRECTS) {
    stage('Test Redirects') {
      dockerContainer.inside(commonStages.DOCKER_ARGS) {
        sh "export IMAGE_TAG=${commonStages.IMAGE_TAG} && docker-compose -p yada up -d && docker-compose -p yada run --rm --entrypoint=npm -e BABEL_ENV=test -e BUILDTYPE=vagovprod yada --no-color run nightwatch:docker -- --env=redirects"
      }
    }
  }
