...
reporsterAgrs="--reporter junit --reporter-options \"mochaFile=test-results/e2e-test-output-[hash].xml\""
...
yarn start-server-and-test watch http://localhost:3001 "cypress run $reporsterAgrs $specArgs"