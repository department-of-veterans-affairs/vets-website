...
if [ "$(find src -name '*.cypress.spec.js' | wc -l)" -eq 0 ]; then
  ...
  reporterArgs="--reporter cypress-multi-reporters --reporter-options \"configFile=config/cypress-reporters.json\" --spec 'src/applications/coronavirus-vaccination/tests/e2e/signup.cypress.spec.js, src/applications/coronavirus-vaccination/tests/e2e/unsubscribe.cypress.spec.js'"
