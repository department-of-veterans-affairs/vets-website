...
/**
 * Node.js 14 compatible script to generate a catalog of all manifest.json files
 * in src/applications and create a markdown table with their metadata.
 */
...
function generateMarkdownTable(manifestData) {
  const header =
    '| Path | App Name | Entry Name | Root URL |\n|------|----------|------------|----------|\n';

  const rows = manifestData
    .filter(data => data !== null)
    .sort((a, b) => a.path.localeCompare(b.path))
    .map(
      data =>
        `| ${data.path} | ${data.appName} | ${data.entryName} | ${
          data.rootUrl
        } |`,
    )
    .join('\n');

  return `# Manifest Catalog\n\nThis file contains a catalog of all manifest.json files found in src/applications.\n\nGenerated on: ${new Date().toISOString()}\n\n${header}${rows}\n`;
}

// Main function
function main() {
  const applicationsDir = path.join(process.cwd(), 'src', 'applications');
  const catalogFile = path.join(process.cwd(), 'manifest-catelog.md');
  ...
  const markdownContent = generateMarkdownTable(validData);

  // Write to catalog file
  fs.writeFileSync(catalogFile, markdownContent, 'utf8');
