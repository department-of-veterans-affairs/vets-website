...
if (batch !== '') {
  const status = runCommandSync(
    `CYPRESS_EVERY_NTH_FRAME=1 DEBUG=cypress-verbose:server:video:* yarn cy:run --browser chrome --headless --reporter cypress-multi-reporters --reporter-options "configFile=config/cypress-reporters.json" --spec '${batch}' --env app_url=${appUrl}`,
  );
  process.exit(status);
} else {
  process.exit(0);
