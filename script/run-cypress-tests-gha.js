...
exec("find src -name '*.cypress.*.js' | tr '\n' ','", function(_err, stdout) {
  ...
  runCommand(
    `CYPRESS_CI=${
      process.env.CI
    } yarn cy:run --browser chrome --reporter cypress-multi-reporters --reporter-options --parallel "configFile=config/cypress-reporters.json" --config baseUrl=http://localhost:${
