...
exec("find src -name '*.cypress.*.js' | tr '\n' ','", function(_err, stdout) {
  ...
  runCommand(
    `CYPRESS_CI=${
      process.env.CI
    } yarn cy:run --browser chrome --reporter cypress-multi-reporters --reporter-options "configFile=config/cypress-reporters.json" --config video=false,baseUrl=http://localhost:${
      process.env.CYPRESS_PORT
    } --port ${Number(process.env.CYPRESS_PORT) - 1} --spec '${tests}'`,
  );
});
