...
if [[ -z "$1" ]] || [[ $1 == -* ]]; then
  BABEL_ENV=test $MOCHA --inspect-brk --opts src/platform/testing/unit/mocha.opts --recursive '$SCRIPT_DIR/../{test,src}/**/*.unit.spec.js?(x)' src/platform/testing/unit/helper.js "$@"
  exit $?
fi

BABEL_ENV=test $MOCHA --inspect-brk --opts src/platform/testing/unit/mocha.opts src/platform/testing/unit/helper.js "$@"
