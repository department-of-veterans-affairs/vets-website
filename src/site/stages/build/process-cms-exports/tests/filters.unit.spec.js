...
describe('CMS export filter helpers', () => {
  describe('getFilter', () => {
    test('should return an array of properties to keep, given the content model type', () => {
      expect(getFilter('paragraph-wysiwyg')).toEqual(['field_wysiwyg']);
    });

    test('should return an empty array if the content model type is not found', () => {
  ...
  describe('getFilteredEntity', () => {
    test("should filter out properties that don't appear in the content model's filters or global whitelist", () => {
      expect(
        getFilteredEntity({
    ...
    test('should use a common blacklist for content models which have no filters specified', () => {
      expect(
        getFilteredEntity({
