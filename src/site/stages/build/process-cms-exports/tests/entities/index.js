/* eslint-disable camelcase */

module.exports = {
  'block_content-alert':
    'block_content.05929395-9414-4d5e-aebd-4473593fe00d.json',
  'block_content-promo':
    'block_content.4a9807dc-7656-4792-b5f9-1a0a80eb2424.json',
  consumer: 'consumer.55ac7544-9db5-4af0-96f1-96c01f1dabe1.json',
  'crop-2_1': 'crop.1d1c14bc-edb7-4dfc-baf5-cf0ad1810da7.json',
  entity_subqueue: 'entity_subqueue.0af013c4-9e81-4a3e-b74f-437536621cc3.json',
  file: 'file.0a3713c8-3533-4676-a894-c79ace11ea53.json',
  'media-document': 'media.10e78352-6dc0-411a-8c41-807f78d9a921.json',
  'media-image': 'media.003339cb-d979-442e-853e-7e56c8f86002.json',
  'media-video': 'media.012ecad1-2030-4ca6-a48f-996921a5816a.json',
  'menu_link_content-burials-and-memorials-benef':
    'menu_link_content.14dc2b61-06dd-4aa1-a993-c9603580ca70.json',
  'node-health_care_region_detail_page':
    'node.00f8a6a5-37a6-478c-b5d0-4eaa87b5bdd1.json',
  'node-person_profile': 'node.01486a41-0a4b-4332-98a8-20794cd24bd0.json',
  'node-page': 'node.01f07d8a-fb30-4ce1-b0e9-9be3d564542d.json',
  'node-regional_health_care_service_des':
    'node.02e024f0-e9c8-4d11-8d71-6a4f3e91366d.json',
  'node-event': 'node.05056017-e2b7-4254-942a-f26e04ad64d8.json',
  'node-support_service': 'node.0590d931-f955-4b85-83f1-2077f6f45225.json',
  'node-outreach_asset': 'node.05b61b5d-9c98-44d8-8b4e-f526116522d7.json',
  'node-health_care_local_health_service':
    'node.0b0e1af6-6ec0-4073-94b1-a99e0383f5c8.json',
  'node-news_story': 'node.0b130154-df25-4284-a561-f5370d589ffd.json',
  'node-health_care_local_facility':
    'node.f875b03e-7ebc-46d9-9320-d412981cd677.json',
  'node-full_width_banner_alert':
    'node.0f3b9c81-762c-4b8a-95e3-dab08a0b9441.json',
  'node-press_release': 'node.2662d0b1-9ee5-45cc-a29f-5192681499ff.json',
  'node-publication_listing': 'node.931072aa-94ed-4072-b3d4-d8735052a073.json',
  'node-health_care_region_page':
    'node.2bddb1a7-6fb1-4503-838d-9c2fcb51c46a.json',
  'node-event_listing': 'node.30295cab-89be-4173-a33c-8c0ca0a85d07.json',
  'node-vamc_operating_status_and_alerts':
    'node.8822bbdd-f5f8-4700-8abb-5566431bd705.json',
  'node-office': 'node.c715d441-b6cb-4067-92eb-0650e97b7d62.json',
  'node-landing_page': 'node.18ca8756-9f78-43ef-95ea-04bd106063c5.json',
  'node-story_listing': 'node.a75c369c-a7f5-402b-aeb9-74ad8d24fd36.json',
  'paragraph-link_teaser':
    'paragraph.c5fec275-9ce2-480c-af68-a6ad65e9ff65.json',
  'paragraph-q_a': 'paragraph.000f52e7-99da-44a1-8b95-2dfe2c2b2682.json',
  'paragraph-q_a_section':
    'paragraph.0046948a-8267-4f38-a02a-a507703fb59a.json',
  'paragraph-wysiwyg': 'paragraph.ffb2a9ae-b630-430f-90b9-43c1e1f68cee.json',
  'paragraph-health_care_local_facility_servi':
    'paragraph.007b6402-f065-4f30-99e0-cac1a2d59b50.json',
  'paragraph-list_of_link_teasers':
    'paragraph.007d3870-ad39-4528-9322-1bcf575d19e8.json',
  'paragraph-collapsible_panel_item':
    'paragraph.00884914-2c00-4111-bdea-76f1deef0646.json',
  'paragraph-staff_profile':
    'paragraph.01200e8f-ba9e-4dfd-89fe-65e9ab81fc21.json',
  'paragraph-alert': 'paragraph.a36d05df-1024-4c7b-9f95-eb06f3392c6c.json',
  'paragraph-downloadable_file':
    'paragraph.01c35dba-24b4-4bc5-b799-6ea359ccfd7b.json',
  'paragraph-table': 'paragraph.01fabcbd-1c2d-4118-912a-4e33fb5569c0.json',
  'paragraph-collapsible_panel':
    'paragraph.6a422b09-790c-4767-a0a6-ac254fe1284a.json',
  'paragraph-process': 'paragraph.b0c6e29f-8add-4313-9235-ad48c9a45414.json',
  'paragraph-number_callout':
    'paragraph.04787f9f-6b8b-4bf5-b71f-28006db691ff.json',
  'paragraph-react_widget':
    'paragraph.0cd20783-f05a-470e-8eb0-ad6f3c597f7a.json',
  'paragraph-expandable_text':
    'paragraph.9bb48828-9a8d-4f48-abde-9c35a4ba3cde.json',
  'paragraph-media': 'paragraph.18827ad2-6420-4c88-8bcc-2328099ccd93.json',
  'paragraph-spanish_translation_summary':
    'paragraph.2a144f80-c8c5-4666-a1e5-03c3b6150aa2.json',
  redirect: 'redirect.0b3efbda-dc16-4da8-9292-cb4bb063e322.json',
  section_association:
    'section_association.4f93a2b2-a118-421e-a62a-c1473e534180.json',
  'taxonomy_term-health_care_service_taxonomy':
    'taxonomy_term.023f7f13-50bb-4eef-9570-78bdad529a3c.json',
  'taxonomy_term-administration':
    'taxonomy_term.2c331a6d-b525-4f0c-8bea-4ecde41c7ef0.json',
  user: 'user.fd71292f-3dc7-409d-b0e8-500204cb005a.json',
};
