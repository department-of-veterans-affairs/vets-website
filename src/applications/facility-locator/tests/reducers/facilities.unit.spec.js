...
describe('facilities reducer', () => {
  ...
  it('should handle fetching state to build a search query object', () => {
    ...
    expect(state.results[0].attributes.classification).to.eql('Test medical center');
    ...
    expect(state.results[0].attributes.address.physical.city).to.eql('New York');
    expect(state.results[0].attributes.address.physical.state).to.eql('NY');
    expect(state.results[0].attributes.address.physical.address1).to.eql('123 East 33rd Street');
    expect(state.pagination.currentPage).to.eql(1);
  });
