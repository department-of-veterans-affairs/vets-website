...
function completeBenefitHistory(client, data) {
  ...
  client
    .clickIf(
      'input[name="root_previousBenefits_view:noPreviousBenefits"]',
      previousBenefits['view:noPreviousBenefits'],
    )
    .clickIf(
      'input[name="root_previousBenefits_disability"]',
      previousBenefits.disability,
    )
    .clickIf('input[name="root_previousBenefits_dic"]', previousBenefits.dic)
    .clickIf(
      'input[name="root_previousBenefits_chapter31"]',
      previousBenefits.chapter31,
    );
  ...
  if (previousBenefits['view:ownServiceBenefits']) {
    client
      ...
      .fill(
        'input[name="root_previousBenefits_ownServiceBenefits"]',
        previousBenefits.ownServiceBenefits,
      );
  }
  ...
  if (previousBenefits['view:claimedSponsorService']) {
    client
      ...
      .clickIf(
        'input[name="root_previousBenefits_chapter35"]',
        previousBenefits.chapter35,
      )
      .clickIf(
        'input[name="root_previousBenefits_chapter33"]',
        previousBenefits.chapter33,
      )
      .clickIf(
        'input[name="root_previousBenefits_transferOfEntitlement"]',
        previousBenefits.transferOfEntitlement,
      )
      .fill(
        'input[name="root_previousBenefits_veteranFullName_first"]',
        previousBenefits.veteranFullName.first,
      )
      .fill(
        'input[name="root_previousBenefits_veteranFullName_middle"]',
        previousBenefits.veteranFullName.middle,
      )
      .fill(
        'input[name="root_previousBenefits_veteranFullName_last"]',
        previousBenefits.veteranFullName.last,
      )
      .selectDropdown(
        'root_previousBenefits_veteranFullName_suffix',
        previousBenefits.veteranFullName.suffix,
      )
      .fill(
        'input[name="root_previousBenefits_view:veteranId_veteranSocialSecurityNumber"]',
        previousBenefits['view:veteranId'].veteranSocialSecurityNumber,
      );
  }

  client.fill(
    'input[name="root_previousBenefits_other"]',
    previousBenefits.other,
  );
}

function completeSponsorService(client, data) {
  const activeDuty = `root_currentlyActiveDuty${
    data.currentlyActiveDuty ? 'Yes' : 'No'
  }`;
  const felony = `root_outstandingFelony${
    data.outstandingFelony ? 'Yes' : 'No'
  }`;
  client
...
function completeSecondaryContact(client, data) {
  ...
  client
    ...
    .fill(
      'input[name="root_secondaryContact_address_street2"]',
      address.street2,
    )
    ...
    .fill(
      'input[name="root_secondaryContact_address_postalCode"]',
      address.postalCode,
    );
}
  ...
  if (!_.isEmpty(data.postHighSchoolTrainings)) {
    ...
    client
      ...
      .waitForElementVisible(
        'input[name="root_postHighSchoolTrainings_0_name"]',
        Timeouts.normal,
      );
    ...
    _.forEach(data.postHighSchoolTrainings, (training, index, allTrainings) => {
      ...
      client
        .fill(
          `input[name="root_postHighSchoolTrainings_${index}_name"]`,
          training.name,
        )
        .fill(
          `input[name="root_postHighSchoolTrainings_${index}_city"]`,
          training.city,
        )
        .selectDropdown(
          `root_postHighSchoolTrainings_${index}_state`,
          training.state,
        );

      if (dateFrom) {
        client.fillDate(
          `root_postHighSchoolTrainings_${index}_dateRange_from`,
          dateFrom,
        );
      }
      if (dateTo) {
      ...
      client
        .fill(
          `input[name="root_postHighSchoolTrainings_${index}_hours"]`,
          training.hours,
        )
        .selectDropdown(
          `root_postHighSchoolTrainings_${index}_hoursType`,
          training.hoursType,
        )
        .fill(
          `input[name="root_postHighSchoolTrainings_${index}_degreeReceived"]`,
          training.degreeReceived,
        )
        .fill(
          `input[name="root_postHighSchoolTrainings_${index}_major"]`,
          training.major,
        );
