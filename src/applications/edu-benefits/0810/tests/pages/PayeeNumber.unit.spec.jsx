...
describe('22-0810 payee number page', () => {
  it('renders the input with the correct label', () => {
    const { container } = renderPage();
    const input = container.querySelector('va-text-input');
    expect(input).to.exist;
    expect(input.getAttribute('label')).to.equal('Payee number');
  });

  it('accepts a valid code without error', () => {
    const { container, unmount } = renderPage({ payeeNumber: 'AB' });
    expect(container.querySelectorAll('[error]')).to.have.length(0);
    unmount();

    const utils2 = renderPage({ payeeNumber: '' });
    expect(utils2.container.querySelectorAll('[error]')).to.have.length(0);
    utils2.unmount();
  });

  it('shows an error when invalid input is given', async () => {
