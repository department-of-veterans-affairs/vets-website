...
describe('Edu 1990e sponsorVeteran', () => {
  const {
    schema,
    uiSchema,
  } = formConfig.chapters.sponsorVeteran.pages.sponsorVeteran;
  it('should render', () => {
  ...
  it('should conditionally require SSN or file number', () => {
    ...
    expect(
      inputs.find(input => input.id === 'root_view:veteranId_vaFileNumber'),
    ).not.to.be.ok;
    ...
    expect(
      errors.find(input =>
        input.id.includes('root_view:veteranId_veteranSocialSecurityNumber'),
      ),
    ).to.be.ok;
    ...
    expect(
      newErrors.find(input =>
        input.id.includes('root_view:veteranId_vaFileNumber'),
      ),
    ).to.be.ok;
  });
