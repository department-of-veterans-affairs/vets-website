...
import manifest from '../../5490/manifest.json';
...
module.exports = E2eHelpers.createE2eTest(client => {
  ...
  if (testData.relationship === 'child') {
    client.waitForElementVisible(
      'label[for="root_benefitsRelinquishedDateMonth"]',
      Timeouts.slow,
    );
  ...
  client.waitForElementVisible(
    'label[for="root_previousBenefits_disability"]',
    Timeouts.slow,
  );
  ...
  client.waitForElementVisible(
    'input[name="root_view:currentSponsorInformation_veteranFullName_first"]',
    Timeouts.slow,
  );
  // Alternatively, we can write a whole new function for this if this is going to
  //  be very specific to the 5490
  EduHelpers.completeVeteranInformation(
    client,
    Object.assign(
      {},
      testData.data,
  ...
  client.waitForElementVisible(
    'select[name="root_highSchool_status"]',
    Timeouts.slow,
  );
