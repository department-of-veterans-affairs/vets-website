...
describe('5490 schema tests', () => {
  ...
  const expectedDataFiles = fs.readdirSync(
    path.join(__dirname, 'expected-data'),
  );
  files
    ...
    .forEach(file => {
      ...
      it(`should validate ${file}`, () => {
        const result = v.validate(JSON.parse(submitData), fullSchema5490);
      ...
      if (expectedDataFiles.indexOf(file) !== -1) {
        it(`submitted data for ${file} should match expected data`, () => {
          ...
          const expectedData = JSON.parse(
            fs.readFileSync(
              path.join(__dirname, 'expected-data', file),
              'utf8',
            ),
          );
