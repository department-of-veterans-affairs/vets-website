...
function completeNewSchool(client, data) {
  ...
  client
    ...
    .setValue(
      'input[name="root_newSchoolAddress_street"]',
      data.newSchoolAddress.street,
    )
    .clearValue('input[name="root_newSchoolAddress_street2"]')
    .setValue(
      'input[name="root_newSchoolAddress_street2"]',
      data.newSchoolAddress.street2,
    )
    .clearValue('input[name="root_newSchoolAddress_city"]')
    .setValue(
      'input[name="root_newSchoolAddress_city"]',
      data.newSchoolAddress.city,
    )
    ...
    .setValue(
      'input[name="root_newSchoolAddress_postalCode"]',
      data.newSchoolAddress.postalCode,
    )
...
function completeOldSchool(client, data) {
  ...
  client
    ...
    .setValue(
      'input[name="root_oldSchool_address_street"]',
      data.oldSchool.address.street,
    )
    .clearValue('input[name="root_oldSchool_address_street2"]')
    .setValue(
      'input[name="root_oldSchool_address_street2"]',
      data.oldSchool.address.street2,
    )
    .clearValue('input[name="root_oldSchool_address_city"]')
    .setValue(
      'input[name="root_oldSchool_address_city"]',
      data.oldSchool.address.city,
    )
    .clearValue('select[name="root_oldSchool_address_state"]')
    .selectDropdown(
      'root_oldSchool_address_state',
      data.oldSchool.address.state,
    )
    .clearValue('input[name="root_oldSchool_address_postalCode"]')
    .setValue(
      'input[name="root_oldSchool_address_postalCode"]',
      data.oldSchool.address.postalCode,
    )
    ...
    .selectDropdown(
      'root_trainingEndDateDay',
      parseInt(dateFields[2], 10).toString(),
    )
...
function completeDetermineEligibility(client) {
  client.waitForElementVisible(
