...
describe('<NoSuggestedAddress />', () => {
  ...
  it('sjould not render addressLine2 is there is no addressLine2', () => {
    ...
    expect(wrapper.find('#entred-address').text()).to.include('123 Main St');
    expect(wrapper.find('#entred-address').text()).to.not.include('Apt 4B');
  });
  it('renders the entered address when deliveryPointValidation is not CONFIRMED', () => {
    expect(wrapper.find('#entred-address').text()).to.not.include(
      '123 Main St Apt 4B',
    );
    expect(wrapper.find('#entred-address').text()).to.include(
      'New York, NY 10001',
    );
  });
