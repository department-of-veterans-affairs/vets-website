...
import { compareObjectsIgnoringExtraKeys } from '../../helpers';

describe('compareObjectsIgnoringExtraKeys', () => {
  it('should return true if objects have the same keys with different values', () => {
    const obj1 = { a: 1, b: 2 };
    const obj2 = { a: 1, b: 3 };
    expect(compareObjectsIgnoringExtraKeys(obj1, obj2)).to.be.true;
  });

  it('should return false if both objects are identical', () => {
    const obj1 = { a: 1, b: 2 };
    const obj2 = { a: 1, b: 2 };
    expect(compareObjectsIgnoringExtraKeys(obj1, obj2)).to.be.false;
  });
  ...
  it('should return false when comparing objects with different keys but ignore extra keys', () => {
    ...
    expect(compareObjectsIgnoringExtraKeys(obj1, obj2)).to.be.false;
    expect(compareObjectsIgnoringExtraKeys(obj1, obj3)).to.be.false;
    expect(compareObjectsIgnoringExtraKeys(obj2, obj3)).to.be.false;
  });
