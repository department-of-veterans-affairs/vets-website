...
function getWarningMessage({
  ...
  if (
    (topBoundaryMet && bottomBoundaryMet) ||
    (leftBoundaryMet && rightBoundaryMet)
  ) {
...
function getDefaultPhotoPosition({ photoData, cropBoxData, containerWidth }) {
  ...
  if (naturalHeight < naturalWidth) {
    return {
      ...
      top: 0,
      left:
        (containerWidth - (cropBoxHeight / oldPhotoHeight) * oldPhotoWidth) / 2,
    };
  }
...
const MoveRotateButton = ({
  ...
  <button
    className={classNames(moveControlClass, { disabled })}
    type="button"
    onClick={onClick}
  >
...
export default class CropperControls extends React.Component {
  ...
  onDone = () => {
    const croppedCanvasOptions =
      this.cropper.getData().width > MAX_CROPPED_HEIGHT_WIDTH
  ...
  getBoundariesMet = () => {
    ...
    const highZoom =
      croppedPhotoWidth < MIN_SIZE || this.state.zoomValue > WARN_RATIO;
  ...
  movePhoto = ({ x = 0, y = 0 } = {}) => {
    ...
    const newPhotoPosition = getNewPhotoPosition({
      photoData,
      boundaries,
      x,
      y,
  ...
  render() {
    ...
    const cropperKey =
      this.state.windowWidth >= LARGE_SCREEN
        ? LARGE_SCREEN
    ...
    return (
      <div className="cropper-container-outer">
        ...
        <div className="cropper-zoom-container">
          <button
            className="cropper-control cropper-control-zoom cropper-control-zoom-out va-button va-button-link"
            type="button"
          ...
          />
          <button
            className="cropper-control cropper-control-zoom cropper-control-zoom-in va-button va-button-link"
            type="button"
        ...
        <div className="cropper-control-container">
          <div className="cropper-control-row">
            <button
              className="cropper-control cropper-control-label-container va-button va-button-link"
              type="button"
              onClick={this.onZoomOut}
            >
          ...
          <div className="cropper-control-row">
            ...
            <MoveRotateButton
              ...
              label="Rotate left"
              iconClassName="fa fa-rotate-right"
            />
          </div>
        </div>
        {this.state.warningMessage && (
          <div className="photo-warning">{this.state.warningMessage}</div>
        )}
