import React from 'react';

const DisputeCharges = () => {
  return (
    <article className="vads-u-padding--0">
      <h2 id="balance-questions">How do I dispute my copay charges?</h2>
      <p>
        You have the right to dispute all or part of your VA copay charges.
        Write a letter explaining why you think the copay charges or balance
        amount may not be correct. To avoid late charges, you’ll need to dispute
        the debt within 30 days of receiving your bill.
      </p>
      <p>
        Mail the letter, or bring it in person, to the business office or health
        administration office at your nearest VA medical center. If you send
        your dispute by mail, please include "Billing Dispute" on the mailing
        envelope.
      </p>
      <p>
        <a className="vads-c-action-link--blue" href="#">
          Find your nearest VA medical center
        </a>
      </p>
      <p>
        <a href="#">Learn more about disputing your copay charges</a>
      </p>
    </article>
  );
};

export default DisputeCharges;
