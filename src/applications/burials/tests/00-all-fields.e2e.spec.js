...
const runTest = E2eHelpers.createE2eTest(client => {
  ...
  client
    .openUrl(`${E2eHelpers.baseUrl}/burials-and-memorials/application/530`)
    .waitForElementVisible('body', Timeouts.normal)
    .assert.title('Apply for Burial Benefits (VA Form 21P-530) | Veterans Affairs')
  ...
  client.waitForElementVisible(
    'input[name="root_toursOfDuty_0_dateRange_fromYear"]',
    Timeouts.normal,
  );
  ...
  // Previous Names page
  client.waitForElementVisible(
    'label[for$="serveUnderOtherNamesYes"]',
    Timeouts.normal,
  );
  ...
  client.waitForElementVisible(
    'label[for="root_view:claimedBenefits_burialAllowance"]',
    Timeouts.normal,
  );
  client.assert.cssClassPresent(
  ...
  if (testData.data['view:claimedBenefits'].burialAllowance) {
    client.waitForElementVisible(
      'label[for="root_burialAllowanceRequested_0"]',
      Timeouts.normal,
    );
  ...
  client.waitForElementVisible(
    'select[name="root_claimantAddress_country"]',
    Timeouts.normal,
  );
  client.assert.cssClassPresent(
