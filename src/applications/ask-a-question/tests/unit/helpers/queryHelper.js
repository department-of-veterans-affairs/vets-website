...
export function getRadioOption(wrapper, optionName, radioName = '') {
    ...
    const element = queriedElements.length === 1
        ? queriedElements[0]
        : queriedElements.find(queriedElement => queriedElement.name.includes(radioName));
...
export function getLabelText(wrapper, labelText, fieldSetName = '') {
    const queriedElements = wrapper.queryAllByLabelText(labelText, {exact: false});

    console.log(queriedElements[0]);
    const element = queriedElements.length === 1
        ? queriedElements
        : queriedElements.find(queriedElement => queriedElement.name.includes(fieldSetName)) || null;
