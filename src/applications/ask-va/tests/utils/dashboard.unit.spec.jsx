...
import { flattenInquiry, categorizeByLOA } from '../../utils/dashboard';
...
describe('flattenInquiry', () => {
  it("hoists nested 'attributes' property into inquiry object", () => {
    const output = flattenInquiry(testInquiries[0]);
  ...
  it('standardizes the inquiry status property', () => {
    expect(flattenInquiry(testInquiries[0]).status).to.equal('In progress');
  });
});

describe('splitInquiresByLOA', () => {
  it('returns ONLY personal and business inquiries', () => {
    const output = categorizeByLOA(testInquiries);
