  ...
  context('Success', () => {
    context('using Lighthouse endpoint', () => {
      it('should dispatch correct actions', async () => {
        ...
        const actionCreator = paymentInformationActions.saveCNPPaymentInformation(
          {
            fields: {
              accountType: 'Checking',
              accountNumber: 'accountNumberFieldValue',
              financialInstitutionRoutingNumber: 'routingNumberFieldValue',
            },
        ...
        // expect the fetch call to be made
        // with the correct request body
        expect(global.fetch.getCall(0).args[1].body).to.be.deep.equal(
          JSON.stringify({
            accountType: 'Checking',
            accountNumber: 'accountNumberFieldValue',
            routingNumber: 'routingNumberFieldValue',
          }),
        );
