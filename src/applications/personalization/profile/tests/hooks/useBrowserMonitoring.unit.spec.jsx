...
describe('profile useBrowserMonitoring hook', () => {
  ...
  const createStoreWithToggles = ({
    ...
    mockStore({
      featureToggles: {
        loading,
        [TOGGLE_NAMES.profileBrowserMonitoring]: toggleValue,
      },
    });
  ...
  describe('initializeBrowserMonitoring', () => {
    ...
    it('passes expected config properties to RUM init', () => {
      ...
      expect(rumConfig.applicationId).to.equal(
        'ed578a2f-233a-4325-a486-d094b369359d',
      );
      expect(rumConfig.clientToken).to.equal(
        'puba5417576e681deca63cfc4af20d16950',
      );
      expect(rumConfig.site).to.equal('ddog-gov.com');
      expect(rumConfig.service).to.equal('va.gov-profile');
