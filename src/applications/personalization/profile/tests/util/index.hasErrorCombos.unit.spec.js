...
describe('hasErrorCombos', () => {
  ...
  context('cases for phone number errors', () => {
    it('return true for Lighthouse night phone error', () => {
      expect(
        hasErrorCombos({
          errors:
            mockDisabilityCompensation.updates.errors.invalidNightPhone.errors,
          errorKeys: [LIGHTHOUSE_ERROR_KEYS.NIGHT_PHONE_NUMBER_INVALID],
        }),
      ).to.be.true;
    });

    it('return true for Lighthouse night area error', () => {
      expect(
        hasErrorCombos({
          errors:
            mockDisabilityCompensation.updates.errors.invalidNightPhoneArea
              .errors,
          errorKeys: [LIGHTHOUSE_ERROR_KEYS.NIGHT_PHONE_AREA_INVALID],
        }),
      ).to.be.true;
    });
