...
const startSaveChannel = (channelId, isAllowed, sensitive) => {
  return {
    type: SAVE_CHANNEL_STARTED,
    payload: { channelId, isAllowed, sensitive },
  };
};

const saveChannelFailed = (channelId, isAllowed, sensitive, errors) => {
  return {
    type: SAVE_CHANNEL_FAILED,
    payload: { channelId, isAllowed, sensitive, errors },
  };
};
...
export const saveCommunicationPreferenceChannel = (channelId, apiCallInfo) => {
  return async dispatch => {
    dispatch(
      startSaveChannel(channelId, apiCallInfo.isAllowed, apiCallInfo.sensitive),
    );
    ...
    try {
      ...
      dispatch(
        saveChannelFailed(
          channelId,
          apiCallInfo.wasAllowed,
          apiCallInfo.sensitive,
          errors,
        ),
      );
...
function communicationItemsReducer(accumulator, item) {
  ...
  const communicationItem = {
    name: item.name,
    // sensitiveIndicator: item.sensitiveIndicator ?? false,
  };
...
function communicationChannelsReducer(accumulator, item) {
  ...
  item.communicationChannels.forEach(channel => {
    ...
    const communicationChannel = {
      ...
      sensitiveIndicator: channel?.sensitiveIndicator,
      defaultSensitiveIndicator: channel?.defaultSensitiveIndicator,
      sensitive:
        channel.communicationPermission?.sensitive ??
        channel?.defaultSensitiveIndicator,
      ui: {
...
export default function reducer(state = initialState, action = {}) {
  switch (action.type) {
    ...
    case SAVE_CHANNEL_SUCCEEDED: {
      ...
      updatedChannel.sensitive = data.sensitive;
    ...
    case SAVE_CHANNEL_FAILED: {
      const { channelId, errors, isAllowed, sensitive } = action.payload;
      const updatedChannel = { ...selectChannelById(state, channelId) };
      updatedChannel.isAllowed = isAllowed;
      updatedChannel.sensitive = sensitive;
      updatedChannel.ui = {
