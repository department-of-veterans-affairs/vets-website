...
const startSaveChannel = channelId => {
  return { type: SAVE_CHANNEL_STARTED, payload: { channelId } };
};

const saveChannelFailed = (channelId, errors) => {
  return { type: SAVE_CHANNEL_FAILED, payload: { channelId, errors } };
};
...
export const saveCommunicationPreferenceChannel = (channelId, apiCallInfo) => {
  return async dispatch => {
    dispatch(startSaveChannel(channelId));
    ...
    try {
      ...
      const errors = error.errors ?? [error];
      dispatch(saveChannelFailed(channelId, errors));
    }
  };
};
...
export default function reducer(state = initialState, action = {}) {
  switch (action.type) {
    ...
    case SAVE_CHANNEL_FAILED: {
      const { channelId, errors } = action.payload;
      const updatedChannel = { ...selectChannelById(state, channelId) };
      updatedChannel.ui = {
        ...updatedChannel.ui,
