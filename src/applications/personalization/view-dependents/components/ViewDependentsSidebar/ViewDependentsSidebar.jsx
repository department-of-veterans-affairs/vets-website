import React, { Component } from 'react';

import ViewDependentsSidebarBlock from './ViewDependentsSidebarBlock';

class ViewDependentsSidebar extends Component {
  render() {
    const firstSidebarBlock = {
      heading: 'When do I need to declare or modify a dependent?',
      content: (
        <>
          <p>
            You need to let VA know when there is a{' '}
            <strong>change in a dependent's status.</strong>
            Changes in status could include:
          </p>
          <ul>
            <li>The birth or adoption of a child</li>
            <li>If you get married or divorced</li>
            <li>If a child becomes seriously disabled</li>
            <li>If your child is over 18 and is not attending school</li>
          </ul>
        </>
      ),
    };

    const secondSidebarBlock = {
      heading: 'What if I need help with my claim?',
      content: (
        <>
          <p>
            You can work with an accredited Veterans Service Officer (VSO). We
            trust these professionals because they are trained and certified in
            the VA claims and appeals process. A VSO can answer your questions
            or even file a claim for you.
          </p>
          <a
            href="#"
            aria-label="Get help filing a claim"
            title="Get help filiing a claim"
          >
            Get help filing a claim
          </a>
        </>
      ),
    };

    const thirdSidebarBlock = {
      heading: 'What if I have questions?',
      content: (
        <>
          <p>
            You can call us at <a href="tel:8008271000">800-827-1000</a>. We're
            here Monday through Friday, 8:00 a.m. to 9:00 p.m. E.T.
          </p>
        </>
      ),
    };

    return (
      <div className="medium-screen:vads-u-padding-left--4">
        <ViewDependentsSidebarBlock
          heading={firstSidebarBlock.heading}
          content={firstSidebarBlock.content}
        />
        <ViewDependentsSidebarBlock
          heading={secondSidebarBlock.heading}
          content={secondSidebarBlock.content}
        />
        <ViewDependentsSidebarBlock
          heading={thirdSidebarBlock.heading}
          content={thirdSidebarBlock.content}
        />
      </div>
    );
  }
}

export default ViewDependentsSidebar;
