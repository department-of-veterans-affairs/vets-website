...
export default function ClaimsListItem({ claim }) {
  ...
  return (
    <div className="claim-list-item-container">
      ...
      <p className="status"><span className="claim-item-label">{moment(claim.attributes.phaseChangeDate).format('MMMM D')} - Update: </span> {listPhase(claim.attributes.phase)}</p>
      ...
      <Link className="usa-button usa-button-primary" href={`/claim-or-appeal-status/your-claims/${claim.id}/status`} onClick={handleViewClaim}>View claim <i className="fa fa-chevron-right"/></Link>
    </div>
  );
}
