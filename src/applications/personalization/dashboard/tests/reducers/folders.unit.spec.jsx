...
describe('folders reducer', () => {
  ...
  it('should delete a folder', () => {
    ...
    expect(Array.from(newState.data.items.values())).to.contain(
      state.data.items.get('test-folder-123'),
    );
    expect(Array.from(newState.data.items.values())).to.contain(
      state.data.items.get('test-folder-789'),
    );
  });
  ...
  it('should set folders fetched from the server', () => {
    ...
    expect(Array.from(newState.data.items.values())).to.eql(
      folders.data.map(folder => folder.attributes),
    );
  });
  ...
  it('should expand and collapse managed folders', () => {
    let newState = foldersReducer(initialState, {
      type: TOGGLE_MANAGED_FOLDERS,
    });
  ...
  it('should update folder counts after moving a message', () => {
        ...
        data: {
          ...
          items: new Map(
            folders.data.map(folder => [
              folderKey(folder.attributes.name),
              folder.attributes,
            ]),
          ),
        },
    ...
    expect(
      newState.data.items.get(folderKey(folders.data[4].attributes.name)).count,
    ).to.equal(0);

    expect(
      newState.data.items.get(folderKey(folders.data[5].attributes.name)).count,
    ).to.equal(3);
  });
});
