...
describe('Verify', () => {
  ...
  it('automatically starts verification if returning from login', () => {
    const mockVerifyHandler = sandbox.spy(
      require('platform/user/authentication/components/VerifyButton'),
      'verifyHandler',
    );

    const urlSearchParams = new URLSearchParams('?postLogin=true');
    ...
    renderInReduxProvider(<Verify />);
    expect(mockVerifyHandler.calledOnce).to.be.true;
  });
