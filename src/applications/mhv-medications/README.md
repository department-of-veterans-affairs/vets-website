...
- [Field Descriptions and Transformations](#field-descriptions-and-transformations)
  ...
  - [MHV API Server-Side Mutations](#mhv-api-server-side-mutations)
...
| `refillStatus` | string | `"active"` | The status of a prescription. Subject to multiple server-side mutations before reaching the FE: `suspended`→`refillinprocess`, `discontinuedEdit`→`discontinued`, `active`→`submitted` (when mhvStatus pending + last request successful), `discontinued`→`transferred` (EHRM migration), `active`/`suspended`→`unknown` (absent from VistA extract). Not the raw VistA status | Used as the primary key for PDF/TXT status definition lookups and for detecting pending medications/renewals (`neworder`/`renew` values). Also used in print layouts. [dispStatus](#disp-status) is preferred for rendering status-specific UI content in the browser |
...
| <span id="dispensed-date">`dispensedDate`</span> | string/date | `"2024-10-31T04:00:00000Z"` | ⚠️ Despite the name, this is actually the pharmacy **release date** (VistA RDT[6]), not the VistA dispensed date (RDT[22]). Renamed from `releaseDateTime` in the MHV API. **Note:** On the **list endpoint**, if refills exist, this is overwritten with the most recent RF record's `releaseDateTime` (not the original RX's). On the **detail endpoint**, the parent RX record uses the original saved value | Use the [sortedDispensedDate](#sorted-dispensed-date) field first, when available. Use this field as a fallback. ProcessList.jsx also pulls from `rxRfRecords[0].dispensedDate` as a source for the most recent dispensed date |
...
| `isRefillable` | boolean | `true` | Whether a prescription can be refilled. Computed server-side with different logic for VistA vs Cerner. For VistA: `active` + refills > 0 (or `activeParked` conditions). The API further restricts: forced `false` if `inCernerTransition` is `true` | **Critical UI field** — controls whether the refill button is rendered, filters which prescriptions appear on the refill page, and gates refill-related UI elements on detail and list pages |
...
| `isTrackable` | boolean | `true` | Whether a prescription has tracking information. `true` when ≥1 tracking record exists in the DB. The `carrier` on each tracking record is derived from the `vamcTrackingNumber` prefix (`92/93/94*`→USPS, `1Z*`→UPS, else Unknown) | This field would most likely not be used in the UI, but can be used for searching or filtering |
...
| `inCernerTransition` (not in use) | boolean | `true` | Computed from institution data: `true` when the facility's `cernerVistaRefillTerminationDate` is in the past. When `true`, forces `isRefillable` to `false` and uses a transition-specific `notRefillableDisplayMessage` | This field is not currently used but could be used to display Cerner-specific messaging for specific prescriptions |
| `notRefillableDisplayMessage` (not in use) | string | `"A refill request cannot be submitted at this time..."` | Computed based on `inCernerTransition`: if `true` → uses `rxrefill.intransition.notrefillable.msg` config property, else → uses `rxrefill.notrefillable.msg`. Both messages injected via `PrescriptionConverter` from application config | This field should not be used in the UI or on VA.gov This was previously used on MHV classic to display the message in the UI |
...
| <span id="dial-cmop-division-phone">`dialCmopDivisionPhone`</span> | string | `"7832721072"` | Computed getter: strips spaces, dashes, parens from `cmopDivisionPhone` and truncates at extension chars (`x`, `X`, `E`, `e`, `#`). Example: `"(783)272-1072 x123"` → `"7832721072"` | Second in the `pharmacyPhoneNumber()` resolution chain — see [cmopDivisionPhone](#cmop-division-phone). Also checked on `rxRfRecords` entries as the final fallback |
| <span id="disp-status">`dispStatus`</span> | string | `"Active"` | Display-formatted status, computed from `refillStatus`: `refillinprocess`→`"Active: Refill in Process"`, `hold`/`providerHold`→`"Active: On Hold"`, `submitted`→`"Active: Submitted"`, `activeParked`→`"Active: Parked"`, `active`+NV source→`"Active: Non-VA"`, others title-cased. Note: `hold` and `providerHold` both display as `"Active: On Hold"` | Used to show the status of a medication in the UI and show status-specific content |
...
| `category` (not in use) | string | `"Documented By VA"` | Always derived from `prescriptionSource`: `NV`→`"Documented by VA"`, anything else→`"Rx Medication"` | This field is not in use and there are no current plans to use this field in the future. This was a field used by MHV classic and will most likely not be used |
...
| `refillStatus` | `refill_status` | `refillStatus` | `status` | 52,100 | RDT[8]; renamed from generic `status`. Subject to DB-level mutations: `suspended`→`refillinprocess`, `discontinuedEdit`→`discontinued`, `active`→`submitted` (when mhvStatus is pending), `discontinued`→`transferred` (when EHRM migration), `active`/`suspended`→`unknown` (when absent from fresh VistA extract) |
| `refillSubmitDate` | `refill_submit_date` | `refillSubmitDate` | `lastRefillSubmittedDate` | N/A | MHV app-level field (not from VistA). Set to `new Date()` (current timestamp) on refill submit. When RF records exist, overwritten by RF[0]'s value; RF records are also backfilled from parent RX during extract if null |
...
| `refillRemaining` | `refill_remaining` | `refillRemaining` | `numberOfRefills` | 52,9 | RDT[11]; renamed from `numberOfRefills`. When refills exist, overwritten by the most recent RF record's count (reflects current remaining, not original) |
...
| `dispensedDate` | `dispensed_date` | `dispensedDate` | `releaseDateTime` | 52,31 | ⚠️ RDT[6]; renamed from `releaseDateTime`. Real VistA dispensedDate is RDT[22] (52,25). On the **list endpoint**, overwritten by most recent RF record's `releaseDateTime`. On the **detail endpoint** (parent RX), uses original saved value |
| `stationNumber` | `station_number` | `stationNumber` | `institution.stationNumber` | Institution lookup | |
| `isRefillable` | `is_refillable` | `isRefillable` | `refillable` | Computed | Jackson serializes `getIsRefillable()` as `isRefillable`. The API helper further restricts: `isRefillable && !inCernerTransition`. VistA logic: `active` + refills > 0, or `activeParked` with specific conditions. Cerner logic differs and is computed at DB ingest time |
| `isRenewable` | `is_renewable` | `isRenewable` | `renewableIndicator` | Computed | Detail endpoint only. DTO stores `renewableIndicator` (string `"1"` or null); API converts to boolean (`"1"` → `true`, else `false`) |
| `isTrackable` | `is_trackable` | `isTrackable` | `isTracking` | Computed | `true` when ≥1 tracking record exists. `carrier` on tracking records is derived from `vamcTrackingNumber` prefix: `92/93/94*`→USPS, `1Z*`→UPS, else Unknown |
| `cmopNdcNumber` | `cmop_ndc_number` | `cmopNdcNumber` | `cmopNdcNumber` | N/A | Not in HL7 RDT; stored in MHV DB |
| `inCernerTransition` (not in use) | `in_cerner_transition` | `inCernerTransition` | none | Computed | Computed getter: `true` when institution's `cernerVistaRefillTerminationDate` is in the past. Affects `isRefillable` (forced `false`) and `notRefillableDisplayMessage` (uses transition-specific config message) |
| `notRefillableDisplayMessage` (not in use) | `not_refillable_display_message` | `notRefillableDisplayMessage` | none | Computed | Computed getter: if `inCernerTransition` → uses `rxrefill.intransition.notrefillable.msg` config, else → `rxrefill.notrefillable.msg` config. Both injected via `PrescriptionConverter` |
...
| `dialCmopDivisionPhone` | `dial_cmop_division_phone` | `dialCmopDivisionPhone` | `dialCmopDivisionPhone` | N/A | Computed getter: strips spaces, dashes, parens from `cmopDivisionPhone` and truncates at `x`/`X`/`E`/`e`/`#` (extension chars). Detail endpoint only |
| `dispStatus` | `disp_status` | `dispStatus` | computed | Computed | Computed getter from `status`: `refillinprocess`→`"Active: Refill in Process"`, `hold`/`providerHold`→`"Active: On Hold"`, `submitted`→`"Active: Submitted"`, `activeParked`→`"Active: Parked"`, `active`+NV source→`"Active: Non-VA"`, others→title-cased |
...
| `prescriptionNumberIndex` | `prescription_number_index` | `prescriptionNumberIndex` | `prescriptionNumberIndex` | RDT[50] | Refill/Partial IEN; detail endpoint only. Defaults to `prescriptionSource` value when null |
...
| `category` | `category` | `category` | `category` | Computed | Detail endpoint only. Always derived from `prescriptionSource`: `NV`→`"Documented by VA"`, anything else→`"Rx Medication"` |
...
| `dispensedDate` | `dispensedDate` | VistA `releaseDateTime` (RDT[6]) — on the list endpoint, overwritten by most recent RF record's `releaseDateTime` | 52,31 / 52.1 | Named "dispensed" but actually the pharmacy **release date**. On the **list endpoint**, if refills exist, this is the most recent RF record's release date (not the original RX's). The detail endpoint restores the original for the parent RX record. The real VistA `dispensedDate` (RDT[22], 52,25) is never exposed. |
...
##### MHV API Server-Side Mutations

The MHV rxrefill Java API applies several layers of data transformation between VistA/DB storage and the API response. Understanding these is important because by the time data reaches the front end, it may not match what's in VistA or the MHV DB.

**RF Record Overwrites** (`PrescriptionServiceImpl.convertToDtos`): When refill records exist, the parent RX's values are overwritten with the most recent RF record's values for: `lastFillDate` (→ `refillDate`), `releaseDateTime` (→ `dispensedDate`), `numberOfRefills` (→ `refillRemaining`), `cmopDivisionPhone`, `lastRefillSubmittedDate` (→ `refillSubmitDate`), and `vamcTrackingNumber`. The original values are saved to `rx*` backup fields (e.g., `rxReleaseDateTime`) — the **detail endpoint** restores these for the parent RX record, but the **list endpoint** uses the overwritten values.

**Status Mutations** (`PrescriptionServiceImpl.setRxStatuses` + `PrescriptionRefreshResponseProcessorImpl`):
| Original Status | Becomes | Condition |
|--|--|--|
| `suspended` | `refillinprocess` | Always |
| `discontinuedEdit` | `discontinued` | Always |
| `active` / `activeParked` | `submitted` | When `mhvStatus` is pending and last request was successful |
| `discontinued` | `transferred` | When reason + comment match EHRM data migration text |
| `active` / `suspended` | `unknown` | When Rx is absent from a fresh VistA extract for that site |

**Non-VA (NV) Filtering**: Non-VA medications with status `discontinued` or `dateOfDeathEntered` are **silently filtered out** and never returned by the API.

**Cerner/Oracle Health Differences**: Cerner prescriptions undergo separate status mapping (e.g., Cerner `"OK"` → MHV `"pending"`), `isRefillable` is computed at DB ingest time (not display time), and `mhvStatus` defaults to `-6` when null.
