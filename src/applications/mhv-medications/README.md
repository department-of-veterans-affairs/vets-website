...
| `institutionId` (not in use) | number | `10` | The auto-generated ID to the facility where the prescription came from | This field is currently not in use and there are no current plans to use this field in the future |
...
| <span id="orderable-item">`orderableItem`</span> | string | `"HALCINONIDE"` | A fallback prescription name for Non-VA prescriptions | Used only as a fallback option if the [prescriptionName](#prescription-name) field does not exist Should continue to be used only as a fallback for Non-VA prescriptions |
