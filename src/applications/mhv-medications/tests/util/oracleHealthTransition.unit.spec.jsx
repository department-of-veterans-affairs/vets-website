...
describe('oracleHealthTransition utilities', () => {
  ...
  describe('formatPhaseDate', () => {
    it('formats ISO dates to readable format and uses fallback for invalid dates', () => {
      expect(formatPhaseDate('2026-04-13', 'Fallback')).to.equal('April 13, 2026');
      expect(formatPhaseDate('March 1, 2026', 'Fallback')).to.equal('March 1, 2026');
  ...
  describe('getCurrentTransitionPhase', () => {
    it('uses backend-provided current phase and fails safe when missing', () => {
      ...
      expect(getCurrentTransitionPhase(migration)).to.equal(MEDICATION_PHASES.FULL_BLOCK);
      ...
      expect(getCurrentTransitionPhase(migrationNoCurrent)).to.equal(MEDICATION_PHASES.PRE_ALERT);

      // Invalid migration data
      expect(getCurrentTransitionPhase(null)).to.equal(MEDICATION_PHASES.PRE_ALERT);
      expect(getCurrentTransitionPhase({})).to.equal(MEDICATION_PHASES.PRE_ALERT);
    });
  });

  describe('isFacilityTransitioning', () => {
    it('returns true when facility ID exists in migrations data, false otherwise', () => {
      expect(isFacilityTransitioning(TRANSITIONING_FACILITY_ID, [mockMichiganMigration])).to.be.true;
      expect(isFacilityTransitioning(NON_TRANSITIONING_FACILITY_ID, [mockMichiganMigration])).to.be.false;
      expect(isFacilityTransitioning(null, [mockMichiganMigration])).to.be.false;
    });
  });
  ...
  describe('shouldBlockRefills', () => {
    it('returns true only when mhv_medications_oracle_health_cutover feature flag enabled AND facility transitioning AND in p4 phase', () => {
      expect(shouldBlockRefills(mockPrescription, true, [createMigrationWithPhase('p4')])).to.be.true;
      expect(shouldBlockRefills(mockPrescription, true, [createMigrationWithPhase('p1')])).to.be.false;
      expect(shouldBlockRefills(mockPrescription, false, [createMigrationWithPhase('p4')])).to.be.false;
    });
  });
  ...
  describe('shouldBlockRenewals', () => {
    it('returns true when mhv_medications_oracle_health_cutover feature flag enabled AND facility transitioning AND in p3 or p4 phase', () => {
      expect(shouldBlockRenewals(mockPrescription, true, [createMigrationWithPhase('p3')])).to.be.true;
      expect(shouldBlockRenewals(mockPrescription, true, [createMigrationWithPhase('p4')])).to.be.true;
      expect(shouldBlockRenewals(mockPrescription, true, [createMigrationWithPhase('p1')])).to.be.false;
    });
  });

  describe('filterPrescriptionsByTransition', () => {
    const mockPrescriptions = [
      { prescriptionId: 1, stationNumber: TRANSITIONING_FACILITY_ID, prescriptionName: 'MEDICATION A' },
      { prescriptionId: 2, stationNumber: NON_TRANSITIONING_FACILITY_ID, prescriptionName: 'MEDICATION B' },
      { prescriptionId: 3, stationNumber: TRANSITIONING_FACILITY_ID_2, prescriptionName: 'MEDICATION C' },
    ];

    it('splits prescriptions into available/blocked arrays based on facility transition status and phase', () => {
      const disabled = filterPrescriptionsByTransition(mockPrescriptions, false, [mockMichiganMigration]);
      expect(disabled.available).to.have.lengthOf(3);
      expect(disabled.blocked).to.have.lengthOf(0);

      const enabled = filterPrescriptionsByTransition(mockPrescriptions, true, [createMigrationWithPhase('p4')]);
    ...
    it('returns empty arrays for null/empty input and fails open (all available) when no migrations', () => {
      ...
      expect(filterPrescriptionsByTransition(null, true, [mockMichiganMigration])).to.deep.equal({ available: [], blocked: [] });
      expect(filterPrescriptionsByTransition([], true, [])).to.deep.equal({ available: [], blocked: [] });
      
      // Empty migrations (fail-safe: all prescriptions available)
      const result = filterPrescriptionsByTransition(mockPrescriptions, true, []);
