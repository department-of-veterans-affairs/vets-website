...
describe('Medications List Page - V2 (Cerner Pilot) Request Refill Button on Card', () => {
  ...
  it('sends POST (not PATCH) for v2 single-refill request', () => {
    const site = new MedicationsSite();
    const listPage = new MedicationsListPage();
    let v1PatchCalled = false;
    ...
    cy.intercept('PATCH', '/my_health/v1/prescriptions/*/refill', () => {
      v1PatchCalled = true;
    });

    cy.intercept('POST', '/my_health/v2/prescriptions/refill', {
      statusCode: 200,
      body: { data: { id: 'test', type: 'prescriptions' } },
    }).as('v2RefillPost');

    listPage.clickRequestRefillButtonOnFirstCard();
    cy.wait('@v2RefillPost');

    cy.then(() => expect(v1PatchCalled).to.be.false);
  });

  it('does not display Request a refill button when refill was submitted within 15 days - v2', () => {
    const site = new MedicationsSite();
    const listPage = new MedicationsListPage();
    ...
    site.login();
    listPage.visitMedicationsListPageURLv2(recentlySubmittedList);
    cy.injectAxe();
    cy.axeCheck('main');
