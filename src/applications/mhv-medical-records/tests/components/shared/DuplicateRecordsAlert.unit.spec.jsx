...
describe('DuplicateRecordsAlert', () => {
  it('renders the alert when visible is true', () => {
    const { getByTestId } = render(<DuplicateRecordsAlert visible />);
    const alert = getByTestId('duplicate-records-info-alert');
    expect(alert).to.exist;
  });

  it('does not render the alert when visible is false', () => {
    const { queryByTestId } = render(<DuplicateRecordsAlert visible={false} />);
    expect(queryByTestId('duplicate-records-info-alert')).to.be.null;
  });

  it('does not render the alert when visible is not provided', () => {
    const { queryByTestId } = render(<DuplicateRecordsAlert />);
    expect(queryByTestId('duplicate-records-info-alert')).to.be.null;
  });
  ...
  it('renders the correct headline', () => {
    const { getByText } = render(<DuplicateRecordsAlert visible />);
