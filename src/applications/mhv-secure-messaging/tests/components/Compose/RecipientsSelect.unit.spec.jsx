...
describe('RecipientsSelect', () => {
  ...
  describe('Recent Recipients Feature', () => {
    const stateWithRecentRecipientsFeature = {
      ...initialState,
      featureToggles: {
    ...
    it('displays recent recipients section when feature flag is enabled and combo box is used', async () => {
      const screen = setup({ state: stateWithRecentRecipientsFeature });

      await waitFor(() => {
        const comboBox = screen.queryByTestId('compose-recipient-combobox');
        expect(comboBox).to.exist;
      });
      ...
      // Should have recent care teams and all care teams sections
      expect(optgroups).to.have.length.above(0);

      // Check for recent care teams optgroup
      const recentOptgroup = Array.from(optgroups).find(
        optgroup => optgroup.label === 'Recent care teams',
      );
      expect(recentOptgroup).to.exist;
    });

    it('does not display recent recipients section when feature flag is disabled', () => {
      ...
      const screen = setup({ state: stateWithoutFeature });
      ...
      if (comboBox) {
        ...
        expect(recentOptgroup).to.not.exist;
      }
    });

    it('does not display recent recipients section for regular select dropdown', () => {
      const stateWithSelectDropdown = {
        ...initialState,
        featureToggles: {
          [FEATURE_FLAG_NAMES.mhvSecureMessagingRecentRecipients]: true,
      ...
      const screen = setup({ state: stateWithSelectDropdown });

      const select = screen.queryByTestId('compose-recipient-select');
