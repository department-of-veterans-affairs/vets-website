...
describe('healthcare-questionnaire -- utils -- appointment parser -- getBookingNoteFromAppointment', () => {
  ...
  it('vdsAppointments is an empty array', () => {
    const result = getBookingNoteFromAppointment({ vdsAppointments: [] });
    expect(result).to.be.null;
  });
  it('vdsAppointments exists, but booking note is empty', () => {
    const result = getBookingNoteFromAppointment({
      vdsAppointments: [{ bookingNote: null }],
    });
    expect(result).to.be.null;
  });
  ...
  it('booking note has an appointment type -- My reason isn’t listed', () => {
    const result = getBookingNoteFromAppointment({
      vdsAppointments: [
        {
          bookingNotes: 'My reason isn’t listed:this is my appointment reason',
        },
      ],
    });
    expect(result.found).to.be.true;
