...
describe('<AppealListItemV2/>', () => {
  ...
  test(
    'should append open class to status-circle div when status active',
    () => {
      const wrapper = shallow(<AppealListItemV2 {...defaultProps}/>);
  ...
  test(
    'should append closed class to status-circle div when status inactive',
    () => {
  ...
  test('should show the right date in the header', () => {
    const wrapper = shallow(<AppealListItemV2 {...defaultProps}/>);
    expect(wrapper.find('h3.claim-list-item-header-v2').render().text()).to.contain('May 1, 2016');
  });

  test('should say "issue" if there is only one issue on appeal', () => {
  ...
  test('should say "issues" if there are multiple issues on appeal', () => {
    const wrapper = shallow(<AppealListItemV2 {...defaultProps}/>);
    const issuesText = wrapper.find('.card-status + p').first().text();
    expect(issuesText).to.contain('Issues');
  });

  test('should create a link to the appeal status page', () => {
    const wrapper = shallow(<AppealListItemV2 {...defaultProps}/>);
    expect(wrapper.find('Link').first().props().to)
      .to.equal(`appeals/${defaultProps.appeal.id}/status`);
  });

  test('should not show the issue text if no description is given', () => {
  ...
  test('should show the issue text if a description is given', () => {
    const wrapper = shallow(<AppealListItemV2 {...defaultProps}/>);
