...
module.exports = E2eHelpers.createE2eTest(client => {
  ...
  client.expect.element('.submit-additional-evidence .usa-alert').to.be.visible;

  // should show a submitted date message
  client.expect
    .element('.submitted-file-list-item:last-child .submission-status')
    .text.to.equal('Submitted');
  // should show a reviewed date message
  client.expect
    .element('.submitted-file-list-item .submission-status')
    .text.to.equal('Reviewed by VA');
