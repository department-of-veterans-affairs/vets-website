...
import LevelTwoLinks from '../../../partials/mobile/header/mega-menu/level-two-links';
...
describe('mega menu - level two links', () => {
  it('should correctly return the benefit hub megamenu given the sectionData', () => {
    ...
    expect(screen.getAllByRole('button')[1].textContent).to.eq('Disability');
    expect(screen.getAllByRole('button')[2].textContent).to.eq(
      'Education and training',
    );
    expect(screen.getAllByRole('button')[3].textContent).to.eq(
      'Careers and employment',
    );
    expect(screen.getAllByRole('button')[4].textContent).to.eq('Pension');
    expect(screen.getAllByRole('button')[5].textContent).to.eq(
      'Housing assistance',
    );
    expect(screen.getAllByRole('button')[6].textContent).to.eq(
      'Life insurance',
    );
    expect(screen.getAllByRole('button')[7].textContent).to.eq(
      'Burials and memorials',
    );
    expect(screen.getAllByRole('button')[8].textContent).to.eq('Records');
