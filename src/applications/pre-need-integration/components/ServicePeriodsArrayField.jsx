...
// vets-website/src/platform/forms-system/src/js/fields/ArrayField.jsx

const { Element } = Scroll;

/* Non-review growable table (array) field */
export default class ServicePeriodArrayField extends React.Component {
  constructor(props) {
    super(props);
    ...
    if (Array.isArray(props.formData)) {
      if (props.formData.length === 1) {
        ...
        showSaveText = props.formData.map(() => false);
      }
    } else {
      showSaveText = [false];
    }
    ...
    this.state = {
      editing: Array.isArray(props.formData)
        ? props.formData.map(
            (item, index) => !errorSchemaIsValid(props.errorSchema[index]),
          )
        : [true],
      showSaveText,
      removing: Array.isArray(props.formData)
        ? props.formData.map(() => false)
        : [false],
    };
  ...
  handleUpdate(index) {
    ...
    // Create a copy of the showSaveText array and update the relevant index
    const updatedShowSaveText = [...this.state.showSaveText];
    updatedShowSaveText[index] = false;

    // Create a copy of the editing array and update the relevant index
    ...
    if (errorSchemaIsValid(this.props.errorSchema[index])) {
      this.setState(
        {
          showSaveText: updatedShowSaveText,
  ...
  handleAdd() {
    ...
    if (errorSchemaIsValid(this.props.errorSchema[lastIndex])) {
      // Create new editing and showSaveText arrays
      const newEditing = [...this.state.editing, true];
      const newShowSaveText = [...this.state.showSaveText, true];

      // Update the state with the new arrays
      const newState = {
        ...this.state,
        editing: newEditing,
        showSaveText: newShowSaveText,
      };
      ...
      <div className={containerClassNames}>
        ...
        <div className="va-growable">
          ...
          {items.map((item, index) => {
            ...
            const updateText = this.state.showSaveText[index]
              ? 'Save'
              : 'Update';
...
ServicePeriodArrayField.propTypes = {
