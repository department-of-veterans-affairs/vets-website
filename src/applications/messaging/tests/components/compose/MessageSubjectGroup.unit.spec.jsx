...
describe('<MessageSubjectGroup>', () => {
  ...
  it('should have the expected classname if no error', () => {
    ...
    expect(tree.props.className).to.equal(
      `${props.cssErrorClass} msg-subject-group`,
    );
  });

  it('should have the expected classname if there is an error', () => {
    const tree = SkinDeep.shallowRender(
      <MessageSubjectGroup {...props} errorMessage="errorMessage" />,
    );

    expect(tree.props.className).to.equal(
      `${
        props.cssErrorClass
      } msg-subject-group usa-input-error msg-compose-error`,
    );
  });
  ...
  it('should pass props to child select element', () => {
    ...
    expect(tree.subTree('MessageCategory').props.categories).to.equal(
      props.categories,
    );
    expect(tree.subTree('MessageSubject').props.charMax).to.equal(
      props.charMax,
    );
  });
});
