...
describe('ThreadHeader', () => {
  ...
  test('should show a message nav', () => {
    const tree = SkinDeep.shallowRender(<ThreadHeader {...props }/>);
    expect(tree.subTree('MessageNav')).to.be.ok;
  });

  test('should hide message nav in case of folder message count errors', () => {
  ...
  test('should show a move button', () => {
    const tree = SkinDeep.shallowRender(<ThreadHeader {...props }/>);
    expect(tree.subTree('MoveTo')).to.be.ok;
  });

  test(
    'should show a button to expand or collapse a thread with multiple messages',
    () => {
      const tree = SkinDeep.shallowRender(<ThreadHeader {...props }/>);
  ...
  test(
    'should not show a button to expand or collapse a thread with only one message',
    () => {
  ...
  test('should show a delete button', () => {
    const tree = SkinDeep.shallowRender(<ThreadHeader {...props }/>);
    expect(tree.subTree('ButtonDelete')).to.be.ok;
  });

  test(
    'should not show delete or move buttons for drafts or sent messages',
    () => {
  ...
  test('should show the subject line in the title', () => {
    const tree = SkinDeep.shallowRender(<ThreadHeader {...props }/>);
    expect(tree.dive(['h2']).text()).to.equal('Subject');
  });

  test(
    'should show only on mobile the subject line for a draft of a new message',
    () => {
      const tree = SkinDeep.shallowRender(
        <ThreadHeader
          {...props }
