...
describe('messages reducer', () => {
  ...
  it('should handle a successful fetch of a message and its thread', () => {
    ...
    expect(state.data.thread).to.eql(
      thread.data.map(msg => msg.attributes).reverse(),
    );

    const { category, messageId, senderId, subject } = message.data.attributes;
    ...
    expect(state.ui.messagesCollapsed).to.eql(
      new Set(thread.data.map(msg => msg.attributes.messageId)),
    );
  });
    ...
    const state = messagesReducer(
      {
        data: { thread: thread.data.map(msg => msg.attributes).reverse() },
        ui: {
          messagesCollapsed: new Set([+thread.data[0].id, +thread.data[1].id]),
        },
      },
      {
        type: TOGGLE_MESSAGES_COLLAPSED,
