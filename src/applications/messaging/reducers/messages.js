  ...
  switch (action.type) {
    case ADD_DRAFT_ATTACHMENTS:
      return set(
        ...
        [...state.data.draft.attachments, ...action.files],
        state,
      );
    ...
    case FETCH_THREAD_MESSAGE_SUCCESS: {
      ...
      const messageIndex = state.data.thread.findIndex(
    ...
    case FETCH_THREAD_SUCCESS: {
      ...
      const messagesCollapsed = new Set(
      ...
      if (!currentMessage.sentDate) {
        ...
        draft.replyMessageId =
          thread.length === 0 ? undefined : thread[thread.length - 1].messageId;
      } else {
    ...
    case TOGGLE_MESSAGES_COLLAPSED: {
      ...
      if (currentCollapsed.size === 0) {
        newMessagesCollapsed = new Set(
          state.data.thread.map(message => message.messageId),
        );
      }
    ...
    case TOGGLE_REPLY_DETAILS:
      return set(
        'ui.replyDetailsCollapsed',
        !state.ui.replyDetailsCollapsed,
        state,
      );
