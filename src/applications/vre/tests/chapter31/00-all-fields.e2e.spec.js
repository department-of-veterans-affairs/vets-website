...
const runTest = E2eHelpers.createE2eTest(client => {
  ...
  // Military history
  client.waitForElementPresent(
    'input[name="root_view:isWorking"]',
    Timeouts.normal,
  );
  client.assert.cssClassPresent(
  ...
  client.waitForElementPresent(
    'input[name="root_yearsOfEducation"]',
    Timeouts.normal,
  );
  client.assert.cssClassPresent(
  ...
  client.waitForElementPresent(
    'select[name="root_disabilityRating"]',
    Timeouts.normal,
  );
  client.assert.cssClassPresent(
  ...
  // Veteran address
  client.waitForElementPresent(
    'select[name="root_veteranAddress_country"]',
    Timeouts.normal,
  );
  client.assert.cssClassPresent(
  ...
  client.waitForElementVisible(
    'label[for="root_dischargeDocuments"]',
    Timeouts.normal,
  );
  client.assert.cssClassPresent(
    '.progress-bar-segmented div.progress-segment:nth-child(7)',
    'progress-segment-complete',
  );
  ...
  client
    .execute(
      "document.getElementById('root_dischargeDocuments').style.display = 'block';",
    )
  ...
  client
    .waitForElementVisible(
      'label#root_dischargeDocuments_add_label',
      Timeouts.slow,
    )
