...
describe('<Main>', () => {
  ...
  test('shows a loading spinner when awaiting response', () => {
    const tree = SkinDeep.shallowRender(<Main {...defaultProps}/>);
    expect(tree.subTree('LoadingIndicator')).to.not.be.false;
  });

  test(
    'shows a loading spinner when letters is available, but address is awaiting response',
    () => {
  ...
  test(
    'shows a loading spinner when letters is awaiting response, but address is available',
    () => {
  ...
  test('renders its children when letters and address are available', () => {
  ...
  test(
    'renders a system down message when letters is available but address is unavailable',
    () => {
  ...
  test('shows a system down message for backend service error', () => {
    const props = _.merge({}, defaultProps, { lettersAvailability: backendServiceError, addressAvailability: available });
    const tree = SkinDeep.shallowRender(<Main {...props}/>);
    expect(tree.subTree('#systemDownMessage')).to.not.be.false;
  });

  test('should show backend authentication error', () => {
  ...
  test('should show system down message for invalid address error', () => {
    const props = _.merge({}, defaultProps, { lettersAvailability: invalidAddressProperty, addressAvailability: available });
    const tree = SkinDeep.shallowRender(<Main {...props}/>);
    expect(tree.subTree('#systemDownMessage')).to.not.be.false;
  });

  test('renders children for letter eligibility errors', () => {
