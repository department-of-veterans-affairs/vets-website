...
describe('10-7959a `titleWithFormDataUI` util', () => {
  const DEFAULT_DATA = { name: 'Cigna' };
  ...
  it('should return a UI schema object with ui:title property', () => {
    const res = titleWithFormDataUI('prescription coverage');
    expect(res).to.have.property('ui:title');
  });

  it('should replace %s placeholder with provider name', () => {
    const uiSchema = titleWithFormDataUI('%s prescription coverage');
  ...
  it('should use custom placeholder token', () => {
    const uiSchema = titleWithFormDataUI('Upload [%p] card', null, {
      placeholder: '%p',
    });
    const result = subject(uiSchema);
...
describe('10-7959a `titleWithRoleUI` util', () => {
...
describe('10-7959a `titleWithNameUI` util', () => {
  const subject = (uiSchema, formData) => {
    const TitleComponent = uiSchema['ui:title'];
    const { container } = render(<div>{TitleComponent({ formData })}</div>);
    return container.textContent;
...
describe('10-7959a `titleWithNameText` util', () => {
  const subject = (title, formData, options) => {
    const titleFn = titleWithNameText(title, options);
    return titleFn(formData);
  };

  it('should return `Your` when certifier role is `applicant`', () => {
    const result = subject('%s identification information', {
  ...
  it('should return full name when `firstNameOnly` option is `false`', () => {
    ...
    expect(result).to.equal('Contact info for John Smith’s');
  });

  it('should fallback to `Beneficiary` when name is missing', () => {
    const result = subject('%s information', {
      certifierRole: 'other',
    });
    expect(result).to.equal('Beneficiary’s information');
  });
