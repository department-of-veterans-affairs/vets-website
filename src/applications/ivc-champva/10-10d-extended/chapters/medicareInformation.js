...
const medicarePlanOver65 = {
  uiSchema: {
    ...
    medicarePlanType: {
      ...radioUI({
        title: `Which Medicare plans does the applicant(s) have?`,
        required: () => true,
        labels: {
          ab:
...
const medicarePlanUnder65 = {
  uiSchema: {
    ...
    medicarePlanType: {
      ...radioUI({
        title: 'Which Medicare plan do the applicant(s) have?',
        required: () => true,
        labels: {
          ab:
            'Original Medicare Parts A and B (Hospital and Medical Insurance)',
...
const medicarePartAPartBEffectiveDatesPage = partC => {
  ...
  if (partC) {
    uiProperties['view:partCExplanation'] = {
      'ui:description': (
        <va-additional-info
          trigger="Why we need Parts A and B information if you have a Part C plan"
