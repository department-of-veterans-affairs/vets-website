...
class SelectArrayItemsBatteriesWidget extends Component {
  handleChecked = (e, supply = {}) => {
    const { selectedProducts, formData } = this.props;
    const { checked } = e.target;
    let updatedSelectedProducts;
    let updatedSelectedRadioButtonForBatteries;
    if (e.target.type === 'checkbox') {
      ...
      if (e.target.value === 'yes') {
        updatedSelectedRadioButtonForBatteries = 'yes';
      } else if (e.target.value === 'no') {
        updatedSelectedRadioButtonForBatteries = 'no';
      }
    }
    const updatedFormData = {
      ...formData,
      selectedProducts: updatedSelectedProducts,
      selectedRadioButtonForBatteries: updatedSelectedRadioButtonForBatteries,
    };
  ...
  render() {
    const {
      supplies,
      selectedProducts,
      selectedRadioButtonForBatteries,
    } = this.props;
    const currentDate = moment();

    return (
      <>
        <h4>Add batteries to your order</h4>
        <p>Do you need to order hearing aid batteries? (*Required)</p>
        <input
          type="radio"
          value="yes"
          id="yes"
          name="batteries"
          checked={selectedRadioButtonForBatteries === 'yes'}
          onChange={e => {
            this.handleChecked(e);
          }}
          required
        />
        <label htmlFor="yes">Yes, I need batteries.</label>
        <input
          type="radio"
          value="no"
          id="no"
          name="batteries"
          checked={selectedRadioButtonForBatteries === 'no'}
          onChange={e => {
            this.handleChecked(e);
          }}
          required
        />
        <label htmlFor="no">No, I don't need batteries.</label>
        {selectedRadioButtonForBatteries === 'yes' && (
          <>
              ...
              supply =>
                supply.productGroup === HEARING_AID_BATTERIES ? (
                  ...
                  >
                    ...
                    {currentDate.diff(supply.nextAvailabilityDate, 'days') <
                    0 ? (
                      <AlertBox
                        ...
                        content={
                          <>
                            ...
                            <p>
                              If you need batteries sooner, call the DLC
                              Customer Service Station at{' '}
                              <a href="tel:303-273-6200">303-273-6200</a> or
                              email{' '}
                              <a href="mailto:dalc.css@va.gov">
                                dalc.css@va.gov
                              </a>
                              .
                            </p>
...
SelectArrayItemsBatteriesWidget.propTypes = {
  ...
  selectedRadioButtonForBatteries: PropTypes.string,
};
...
const mapStateToProps = state => ({
  ...
  selectedRadioButtonForBatteries:
    state.form?.data?.selectedRadioButtonForBatteries,
});
