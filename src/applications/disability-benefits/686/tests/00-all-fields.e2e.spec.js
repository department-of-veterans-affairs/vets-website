...
const runTest = E2eHelpers.createE2eTest(
  (client) => {
    ...
    if (process.env.BUILDTYPE !== 'production') {
      ...
      client.waitForElementVisible('legend#root_maritalStatus-label', Timeouts.normal);
      ...
      testData.data.marriages.forEach((marriageData, index) => {
        ...
        client.axeCheck('.main')
          .click('.form-panel .usa-button-primary');
        E2eHelpers.expectNavigateAwayFrom(client, `/household/marriages/${index}`);
      });

      // spouse info
      client.waitForElementVisible('legend#root_spouseDateOfBirth-label', Timeouts.normal);
      ...
      // dependent info
      client.waitForElementVisible('input#root_childSocialSecurityNumber', Timeouts.normal);
      client.assert.cssClassPresent('.progress-bar-segmented div.progress-segment:nth-child(1)', 'progress-segment-complete');
      PageHelpers.completeDependentInfo(client, testData.data, 0);
      client.axeCheck('.main')
        .click('.form-panel .usa-button-primary');
      E2eHelpers.expectNavigateAwayFrom(client, '/unmarried-children/information/0');
      ...
      client.axeCheck('.main')
        .click('.form-panel .usa-button-primary');
      E2eHelpers.expectNavigateAwayFrom(client, '/household/dependents/unmarried-children/address/0');
