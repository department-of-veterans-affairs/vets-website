...
function completeClaimantInformation(client, data) {
  client
    ...
    .fill('input[name="root_view:applicantInfo_ssn"]', data.claimantSocialSecurityNumber)
    .fill('input[name="root_view:applicantInfo_claimantEmail"]', data.claimantEmail);
}
...
function completeMarriage(client, marriageData) {
  client
    ...
    .fill('input[name="root_locationOfMarriage"]', marriageData.locationOfMarriage);
  if (marriageData['view:pastMarriage']) {
    client
      .fillDate('root_view:pastMarriage_dateOfSeparation', marriageData['view:pastMarriage'].dateOfSeparation)
      .fill('input[name="root_view:pastMarriage_locationOfSeparation"]', marriageData['view:pastMarriage'].locationOfSeparation);
  }
}
...
function completeDependentsInformation(client, data) {
  client
    .selectYesNo('root_view:hasUnmarriedChildren', data['view:hasUnmarriedChildren']);
...
function completeDependentAddressInfo(client, data, index) {
  ...
  if (!data.dependents[index].childInHousehold) {
    client
      .fillAddress('root_childInfo_childAddress', data.dependents[index].childAddress)
      .fill('input#root_childInfo_personChildLiveWith_firstName', data.dependents[index].personWhoLivesWithChild.first)
      .fill('input#root_childInfo_personChildLiveWith_lastName', data.dependents[index].personWhoLivesWithChild.last);
  }
}
