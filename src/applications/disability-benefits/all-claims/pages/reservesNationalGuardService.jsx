...
function validateMilitaryCity(errors, city, formData) {
  const isMilitaryState = MILITARY_STATE_VALUES.includes(
    formData.serviceInformation.reservesNationalGuardService.unitAddress.state || ''
  );
...
function validateMilitaryState(errors, state, formData) {
  const isMilitaryCity = MILITARY_CITIES.includes(
    formData.serviceInformation.reservesNationalGuardService.unitAddress.city.trim().toUpperCase()
  );
...
const updateStates = (form) => {
  const currentCity = get(form, 'serviceInformation.reservesNationalGuardService.unitAddress.city', '').trim().toUpperCase();
  ...
  serviceInformation: {
    reservesNationalGuardService: {
      ...
      unitAddress: {
        ...
        state: {
          'ui:title': 'State',
          'ui:required': formData => get(formData, 'serviceInformation.reservesNationalGuardService.unitAddress.country', USA) === USA,
          'ui:options': {
            hideIf: formData => get(formData, 'serviceInformation.reservesNationalGuardService.unitAddress.country', USA) !== USA,
