...
//   const ratedIncreaseSet = new Set(ratedIncreaseNames);

//   let ratedChanged = false;
//   const updatedRated = ratedDisabilities.map(rd => {
//     if (ratedIncreaseSet.has(rd.name)) {
//       if (!rd['view:selected']) {
//         ratedChanged = true;
...
//   // Remove rated-increase entries from newDisabilities
//   const filteredNewDisabilities = newDisabilities.filter(
//     nd =>
//       !(
//         nd?.cause === 'WORSENED' &&
//         nd?.condition === 'Rated Disability' &&
//         ratedIncreaseSet.has(nd?.ratedDisability)
//       ),
//   );

//   if (!ratedChanged && filteredNewDisabilities.length === newDisabilities.length) {
//     return formData;
//   }
