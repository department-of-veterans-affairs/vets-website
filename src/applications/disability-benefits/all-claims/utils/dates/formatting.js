 ...
 * @private
 */
const safeDateFns = (date, dateFormat) => {
  if (!date) return null;
 ...
 * @param {string} dateFormat - Optional format string (defaults to DATE_FORMAT)
 * @returns {string} Formatted date or 'Unknown' if invalid
 */
export const formatDate = (date, dateFormat = 'LLLL d, yyyy') => {
  const fnsDate = safeDateFns(date);
  return fnsDate ? format(fnsDate, dateFormat) : 'Unknown';
};
...
export const formatDateRange = (
  ...
  const fromDate = formatDate(dateRange.from, dateFormat);
  const toDate = formatDate(dateRange.to, dateFormat);
...
export const formatMonthYearDate = (rawDate = '') => {
  if (!rawDate) return '';
  const date = safeDateFns(rawDate);
  if (!date) return '';
...
export const isValidFullDate = dateString => {
  ...
  if (dateString !== format(date, 'yyyy-MM-dd')) return false;
...
export const validateAge = (err, fieldData, formData) => {
  ...
  const dateOfBirthFns = safeDateFns(dateOfBirth);
  const fieldDateFns = safeDateFns(fieldData);

  if (!dateOfBirthFns || !fieldDateFns) return;

  const thirteenthBirthday = add(dateOfBirthFns, { years: 13 });

  if (isBefore(fieldDateFns, thirteenthBirthday)) {
...
export const validateSeparationDate = (err, fieldData, formData) => {
  ...
  const separationDate = safeDateFns(fieldData);
  ...
  servicePeriods.forEach(period => {
    const startDate = safeDateFns(period.dateRange?.from);

    if (startDate && isBefore(separationDate, startDate)) {
...
export const validateServicePeriod = (errors, fieldData) => {
  ...
  const startDate = safeDateFns(from);
  const endDate = safeDateFns(to);
...
export const isLessThan180DaysInFuture = (errors, fieldData) => {
  const enteredDate = safeDateFns(fieldData);
  ...
  if (!isAfter(enteredDate, today)) {
    errors.addError('Enter a future separation date');
  } else if (isAfter(enteredDate, in180Days)) {
...
export const isWithinRange = (dates, range) => {
  ...
  const startDate = safeDateFns(range.from);
  const endDate = safeDateFns(range.to);
  ...
  return datesToCheck.every(date => {
    const dateToValidate = safeDateFns(date);
    return (
      dateToValidate &&
...
export const parseDate = (dateString, dateFormat) => {
  return safeDateFns(dateString, dateFormat);
};
...
export const parseDateWithTemplate = dateString => {
  return safeDateFns(dateString, 'yyyy-MM-dd');
};
...
export const isBddClaimValid = separationDate => {
  const separationDateFns = safeDateFns(separationDate);
  if (!separationDateFns) return false;

  const today = startOfDay(new Date());
  const daysUntilSeparation = differenceInDays(separationDateFns, today);
...
export const getBddSeparationDateError = separationDate => {
  const separationDateFns = safeDateFns(separationDate);
  if (!separationDateFns) return 'Please provide a valid separation date';
...
export const findEarliestServiceDate = servicePeriods => {
  ...
  const validPeriods = servicePeriods
    ...
    .map(period => parse(period.dateRange.from, 'yyyy-MM-dd', new Date()))
