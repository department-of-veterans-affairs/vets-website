...
describe('Disability benefits 526EZ -- Partial date validations', () => {
  ...
  describe('validateYearOnlyPartialDate', () => {
    ...
    it('should skip validation for undefined input', () => {
      validateYearOnlyPartialDate(errorsSpy, undefined);

      expect(errorsSpy.addError.called).to.be.false;
    });

    it('should allow any valid 4-digit year', () => {
      validateYearOnlyPartialDate(errorsSpy, '1900');
      expect(errorsSpy.addError.called).to.be.false;
    });

    it('should allow current and recent years', () => {
      validateYearOnlyPartialDate(errorsSpy, '2020');
      expect(errorsSpy.addError.called).to.be.false;
    });

    it('should allow past years in valid range', () => {
      validateYearOnlyPartialDate(errorsSpy, '1950');
      expect(errorsSpy.addError.called).to.be.false;
    });

    it('should handle numeric year input', () => {
      validateYearOnlyPartialDate(errorsSpy, 2020);
      expect(errorsSpy.addError.called).to.be.false;
    });
