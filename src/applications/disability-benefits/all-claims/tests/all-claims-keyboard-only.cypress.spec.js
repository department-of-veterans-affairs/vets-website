  ...
  */
  ...
  it('navigate through a maximal form', () => {
    cy.get('@testData').then(() => {
      ...
      // Something about these next steps causes the test runner on CI to time
      // out (after 90 minutes). Can't figure out why. 10 points to whoever
      // cracks it.

      // Also, Note that this stanza uses `data` which needs to be passed into the
      // `.then` callback: cy.get('@testData').then(({ data }) => { ...

      // // 1. Can update existing info
      // idRoot = '#root_phoneAndEmail';
