...
describe('ToxicExposureChoicePage', () => {
  ...
  describe('Display modal for deleting already entered toxic exposure data', () => {
    ...
    describe('when "none" is selected and existing toxic exposure data exists', () => {
      ...
      it('displays the modal for other exposures', async () => {
        const data = {
          ...baseDataNoneSelected,
          toxicExposure: {
            ...baseDataNoneSelected.toxicExposure,
      ...
      it('displays the modal for other herbicide locations', async () => {
        const data = {
          ...baseDataNoneSelected,
          toxicExposure: {
            ...baseDataNoneSelected.toxicExposure,
            otherHerbicideLocations: 'Thailand',
          },
        };
      ...
      it('displays the modal for specify other exposures', async () => {
        const data = {
          ...baseDataNoneSelected,
          toxicExposure: {
            ...baseDataNoneSelected.toxicExposure,
  ...
  describe('Modal delete action selection', () => {
    ...
    describe('When the Confirm button is clicked', () => {
      const fullToxicExposureMetadata = {
        ...
        otherHerbicideLocations: 'Thailand military base',
      ...
      it('deletes all toxic exposure data except conditions when "none" is selected', async () => {
        ...
        await waitFor(() => {
          ...
          // Verify only conditions remain
          expect(calledData.toxicExposure).to.deep.equal({
            conditions: { none: true },
          });

          // Verify all other TOXIC_EXPOSURE_ALL_KEYS are deleted
          TOXIC_EXPOSURE_ALL_KEYS.forEach(key => {
            if (key !== 'conditions') {
              expect(calledData.toxicExposure[key]).to.be.undefined;
            }
          });
      ...
      it('deletes all toxic exposure data when nothing is selected', async () => {
        ...
        await waitFor(() => {
          ...
          // Verify only conditions remain (empty)
  ...
  describe('TOXIC_EXPOSURE_ALL_KEYS validation', () => {
    it('deleteToxicExposureData function removes all keys from TOXIC_EXPOSURE_ALL_KEYS except conditions', () => {
      // Create data with all possible toxic exposure keys
      const fullData = {
        newDisabilities: [{ condition: 'Test Condition' }],
        toxicExposure: {
          conditions: { testcondition: true },
          gulfWar1990: { bahrain: true },
          gulfWar1990Details: { bahrain: { startDate: '1991-01-01' } },
          gulfWar2001: { afghanistan: true },
          gulfWar2001Details: { afghanistan: { startDate: '2001-10-01' } },
          herbicide: { vietnam: true },
          herbicideDetails: { vietnam: { startDate: '1968-01-01' } },
          otherHerbicideLocations: 'Thailand',
          otherExposures: { asbestos: true },
          otherExposuresDetails: { asbestos: { startDate: '1980-01-01' } },
          specifyOtherExposures: 'Lead exposure',
          // Add any unexpected keys to verify they aren't deleted
          unexpectedKey: 'should remain',
        },
      };
      ...
      // Trigger the deleteToxicExposureData function by simulating the modal confirm
      // This tests the actual implementation in the component
      const component = document.querySelector('form');
      expect(component).to.exist;

      // Verify TOXIC_EXPOSURE_ALL_KEYS contains all expected keys
      expect(TOXIC_EXPOSURE_ALL_KEYS).to.include('conditions');
      expect(TOXIC_EXPOSURE_ALL_KEYS).to.include('gulfWar1990');
      expect(TOXIC_EXPOSURE_ALL_KEYS).to.include('gulfWar1990Details');
      expect(TOXIC_EXPOSURE_ALL_KEYS).to.include('gulfWar2001');
      expect(TOXIC_EXPOSURE_ALL_KEYS).to.include('gulfWar2001Details');
      expect(TOXIC_EXPOSURE_ALL_KEYS).to.include('herbicide');
      expect(TOXIC_EXPOSURE_ALL_KEYS).to.include('herbicideDetails');
      expect(TOXIC_EXPOSURE_ALL_KEYS).to.include('otherHerbicideLocations');
      expect(TOXIC_EXPOSURE_ALL_KEYS).to.include('otherExposures');
      expect(TOXIC_EXPOSURE_ALL_KEYS).to.include('otherExposuresDetails');
      expect(TOXIC_EXPOSURE_ALL_KEYS).to.include('specifyOtherExposures');
    });

    it('hasToxicExposureData function checks all keys from TOXIC_EXPOSURE_ALL_KEYS', () => {
      // Test with empty toxic exposure
      const emptyData = {
        toxicExposure: {
          conditions: {},
        },
      };
      const { container: emptyContainer } = renderPage({ data: emptyData });
      expect(emptyContainer).to.exist;

      // Test with each key having data
      TOXIC_EXPOSURE_ALL_KEYS.forEach(key => {
        if (key === 'conditions') return; // Skip conditions as it's handled separately

        const testData = {
          toxicExposure: {
            [key]: key.includes('Details')
              ? { someLocation: {} }
              : 'test value',
          },
        };
