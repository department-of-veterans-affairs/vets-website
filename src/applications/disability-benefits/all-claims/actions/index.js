...
export const ITF_CREATION_ENQUEUED = 'ITF_CREATION_ENQUEUED';

// [wipn8923] fetch itf method!
...
export function createNonBlockingITF() {
  return dispatch => {
    dispatch({ type: ITF_CREATION_INITIATED });

    return apiRequest('/intent_to_file/compensation', { method: 'POST' })
      .then(({ data }) => dispatch({ type: ITF_CREATION_ENQUEUED, data }))
      .catch(() => {
        Sentry.captureMessage('itf_creation_failed');
        dispatch({ type: ITF_CREATION_FAILED });
      });
