...
describe('HCA-Keyboard-Only', () => {
  ...
  it('should navigate and input maximal data using only a keyboard', () => {
    ...
    cy.tabToElementAndPressSpace(selector('isAmericanIndianOrAlaskanNative'));
    cy.tabToElementAndPressSpace(selector('isAsian'));
    cy.tabToElementAndPressSpace(selector('isBlackOrAfricanAmerican'));
    cy.tabToElementAndPressSpace(selector('isSpanishHispanicLatino'));
    cy.tabToElementAndPressSpace(
      selector('isNativeHawaiianOrOtherPacificIslander'),
    );
    cy.tabToElementAndPressSpace(selector('isWhite'));
    cy.tabToContinueForm();
    ...
    cy.tabToElementAndPressSpace(selector('isFormerPow'));
    cy.tabToElementAndPressSpace(selector('postNov111998Combat'));
    cy.tabToElementAndPressSpace(selector('disabledInLineOfDuty'));
    cy.tabToElementAndPressSpace(selector('swAsiaCombat'));
    cy.tabToContinueForm();
    ...
    cy.tabToElementAndPressSpace(selector('exposureToAirPollutants'));
    cy.tabToElementAndPressSpace(selector('exposureToAsbestos'));
    cy.tabToElementAndPressSpace(selector('exposureToChemicals'));
    cy.tabToElementAndPressSpace(selector('exposureToContaminatedWater'));
    cy.tabToElementAndPressSpace(selector('exposureToMustardGas'));
    cy.tabToElementAndPressSpace(selector('exposureToOccupationalHazards'));
    cy.tabToElementAndPressSpace(selector('exposureToRadiation'));
    cy.tabToElementAndPressSpace(selector('exposureToShad'));
    cy.tabToElementAndPressSpace(selector('exposureToWarfareAgents'));
    ...
    cy.selectRadioFromData('[name="root_discloseFinancialInformation"]', 'Y');
    cy.tabToContinueForm();
    ...
    cy.selectDropdownFromData(selector('view:facilityState'), facilityState);
    cy.wait('@getFacilities');
    cy.selectDropdownFromData(selector('vaMedicalFacility'), vaMedicalFacility);
