...
describe('HCA helpers', () => {
  describe('expensesLessThanIncome', () => {
    test('should return true if expenses less than income', () => {
      const formData = {
        veteranNetIncome: 3,
    ...
    test('should return false if expenses greater than income', () => {
      const formData = {
        veteranNetIncome: 3,
    ...
    test('should include income from dependents', () => {
      const formData = {
        deductibleMedicalExpenses: 2,
        dependents: [{
          grossIncome: 3
    ...
    test('should include spouse income', () => {
      const formData = {
        deductibleMedicalExpenses: 2,
        'view:spouseIncome': {
          spouseGrossIncome: 3
    ...
    test('should show only if last non-zero expense amount', () => {
      const formData = {
        dependents: [{
          grossIncome: 3
        }],
    ...
    test(
      'should show warning just under last field if all expenses are filled',
      () => {
        const formData = {
          dependents: [{
            grossIncome: 3
  ...
  describe('getCSTOffset', () => {
    test('should return -300 if is daylight savings time', () => {
      expect(getCSTOffset(true)).to.equal(-300);
    });
    test('should return -360 if is not daylight savings time', () => {
      expect(getCSTOffset(false)).to.equal(-360);
    });
  });
  describe('getOffsetTime', () => {
    test('should convert an offset number of minutes into milliseconds', () => {
      expect(getOffsetTime(1)).to.equal(60000);
    });
  });
  describe('getAdjustedTime', () => {
    test('should determine utc time', () => {
      expect(getAdjustedTime(1, 1)).to.equal(2);
    });
  });
  describe('isAfterCentralTimeDate', () => {
    test(
      'should return true if the discharge date is after the Central Time reference date',
      () => {
        expect(isAfterCentralTimeDate('9999-12-24')).to.be.true;
      }
    );
    test(
      'should return false if the discharge date is not after the Central Time reference date',
      () => {
