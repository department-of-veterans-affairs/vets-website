...
export const veteranMailingAddress = {
  mailingAddress: {
    path: 'veteran-information/mailing-address',
    title: 'Mailing address',
    initialData: {},
    depends: formData => !formData['view:doesPermanentAddressMatchMailing'],
    uiSchema: {
      'ui:description': PrefillMessage,
      veteranMailingAddress: merge(addressUI('Permanent address', true), {
        street: {
          'ui:errorMessages': {
            pattern:
              'Please provide a valid street. Must be at least 1 character.',
    ...
    schema: {
      type: 'object',
      properties: {
        veteranMailingAddress: merge(addressSchema(fullSchemaHca, true), {
          properties: {
            street: {
              minLength: 1,
              maxLength: 30,
