...
import testData from '../fixtures/data/maximal-test.json';
...
describe('EvidencePrivateRecords', () => {
  ...
  it('should render the proper content', () => {
    ...
    expect($$('#add-facility-name', container)).to.exist;
  ...
  it('should render the proper content with no data given ', () => {
    const { container } = render(setup({ data: null }));
    const h3s = $$('h3', container);

    verifyHeader(h3s, 0, 'Add the first provider where you received treatment');
  ...
  it('should update facility name', async () => {
    ...
    expect(setDataSpy.args[0][0].providerFacility[0]).to.deep.equal({
      ...testData.data.providerFacility[0],
