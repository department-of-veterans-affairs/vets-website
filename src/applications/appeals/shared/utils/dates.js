...
const getAbbreviatedMonth = date => {
  const monthText = format(date, 'MMM');
  return monthText === 'Sep' ? 'Sept.' : `${monthText}.`;
};
...
export const formatDateToReadableString = date => {
  ...
  // Use standard abbreviation with period, handle September special case
  const abbreviatedMonth = getAbbreviatedMonth(date);
...
export const formatMonthYearToReadableString = date => {
  ...
  // Use standard abbreviation with period, handle September special case
  const abbreviatedMonth = getAbbreviatedMonth(dateObj);
