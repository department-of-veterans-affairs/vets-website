...
export const cleanUpAnswers = (
  ...
  // Get the questions coming up after this one
  // We're here because the user has backed up and changed their answer and we want
  // to clear out all answers ahead of this question
  const questionsAhead = allQuestionShortNames.slice(currentQuestionIndex + 1);

  updateCleanedFormStore({
    ...questionsAhead.map(question => ({ [question]: null })),
  });
};
