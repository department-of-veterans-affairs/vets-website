...
describe('Locator url and parameters builder', () => {
  ...
  it('should build VA request with type=officer', () => {
    ...
    expect(test).to.eql(
      `${
        environment.API_URL
      }/services/veteran/v0/accredited_representatives?address=43210&lat=40.17887&long=-99.27246&name=test&page=1&per_page=10&sort=distance_asc&type=officer`,
    );
  });
  ...
  it('should exclude null params from request', () => {
    ...
    expect(test).to.eql(
      `${
        environment.API_URL
      }/services/veteran/v0/accredited_representatives?page=2&per_page=7&sort=distance_asc`,
    );
  });
});
