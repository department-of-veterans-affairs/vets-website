...
describe('Main', () => {
  ...
  test('should show data when service is available', () => {
    /*
  ...
  test('should show loading spinner when waiting for response', () => {
    const props = _.merge({}, defaultProps, { availability: 'awaitingResponse' });
    const tree = SkinDeep.shallowRender(<Main {...props}/>);
    expect(tree.subTree('LoadingIndicator')).to.be.ok;
  });

  test('should show system down message for backend service error', () => {
  ...
  */
  ...
  test('should show System Down warning when record not found', () => {
    const props = _.merge({}, defaultProps, { availability: 'noChapter33Record' });
    const tree = SkinDeep.shallowRender(<Main {...props}/>);
