  ...
  render() {
    ...
    const crumbs = [
      <a
        ...
        key="home"
        onClick={() => {
          recordEvent({ event: 'nav-breadcrumb' });
        }}
      >
        Home
      </a>,
      <a href="/health-care/" key="healthcare">
        Health Care
      </a>,
    ];
    ...
    if (pathname.match(/\/\d+$/)) {
      ...
      if (prescription) {
        const prescriptionId = _.get(prescription, [
          'rx',
          'attributes',
          'prescriptionId',
        ]);
        ...
        crumbs.push(
          <Link to={`/${prescriptionId}`} key="history">
            {prescriptionName}
          </Link>,
        );
      }
