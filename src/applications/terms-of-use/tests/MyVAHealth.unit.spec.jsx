...
describe('MyVAHealth', () => {
  ...
  it('should retry provisioning up to 3 times before redirecting', async () => {
  ...
  it('should redirect to error page 111 if provisioning fails after 3 retries', async () => {
    global.window.location = `https://dev.va.gov/terms-of-use/myvahealth/?ssoeTarget=${ssoeTarget}`;
  ...
  it('should wait 500ms before checking provisioning after TOU acceptance', async () => {
      ...
      rest.put(
        ...
        (_, res, ctx) => {
          if (!provisioningChecked) {
            return res(
              ctx.status(400),
              ctx.json({ error: 'Provisioning too early' }),
            );
          }
          return res(ctx.status(200), ctx.json({ provisioned: true }));
        },
    ...
    const { findByTestId } = renderInReduxProvider(<MyVAHealth />, {
      initialState: { featureToggles: { terms_of_use: true } },
    });
    ...
    await waitFor(() => {
      expect(global.window.location).to.eql(ssoeTarget);
      expect(provisioningChecked).to.be.true;
    });
  });

  // Retains all previous tests to ensure old behavior is still tested
  it('should redirect to error page 110 if any other error occurs', async () => {
    const startingLocation = `https://dev.va.gov/terms-of-use/myvahealth/?ssoeTarget=${ssoeTarget}`;
    global.window.location = startingLocation;
