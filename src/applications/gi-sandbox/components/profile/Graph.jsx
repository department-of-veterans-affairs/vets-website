...
class Graph extends React.Component {
  ...
  render() {
    ...
    const HollowUnderbar = ({ top }) => (
      <path
        d={`M80.544 28.8h128.912V3.2H80.544v25.6zM78 ${top ||
          '0'}h134v32H78V0z`}
        fill="#F1F1F1"
        fillRule="nonzero"
      />
    );
    const SolidUnderbar = ({ top }) => (
      <path fill="#F1F1F1" d={`M78 ${top || 0}h134v32H78z`} />
    );
    const Underbar = ({ top, value, children }) =>
      value ? (
        <g>
          <SolidUnderbar top={top} />
          {children}
        </g>
      ) : (
        <HollowUnderbar top={top} />
      );
          ...
          <g fill="none" fillRule="evenodd">
            ...
            <Underbar top="0" value={this.format(veterans) !== 'No Data'}>
              <ValueBar
                top="0"
                color="#02BFE7"
                percent={veterans.value ? veterans.value : 1}
              />
            </Underbar>
            ...
            {averagePercent ? (
              <AverageMark
                ...
                text={this.format(averageObject)}
              />
            ) : null}
          </g>
        </svg>
