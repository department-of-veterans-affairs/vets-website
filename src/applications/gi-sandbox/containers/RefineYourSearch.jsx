...
export function RefineYourSearch({
  ...
  const renderCountryFilter = () => {
    const options = search.name.facets.country.map(country => ({
  ...
  const renderStateFilter = () => {
    const options = Object.keys(search.name.facets.state).map(state => ({
      optionValue: state,
  ...
  const renderTypeOfSchool = () => {
    const options = [
      ...Object.keys(search.name.facets.type).map(type => ({
        optionValue: type,
        optionLabel: type,
      })),
    ];
