...
describe('characterReferencesPages', () => {
  ...
  const getRadioError = container => {
    ...
    if (group) {
      const attr = group.getAttribute('error');
      const prop = group.error;
      if (typeof attr === 'string' && attr.length) return attr;
      if (typeof prop === 'string' && prop.length) return prop;
    }
    const fallback = container.querySelector('[error]');
    if (fallback) {
      const fAttr = fallback.getAttribute('error');
      if (typeof fAttr === 'string' && fAttr.length) return fAttr;
      if (typeof fallback.error === 'string' && fallback.error.length)
  ...
  describe('characterReferences count validation', () => {
    ...
    it('shows a min error when there are 0 references', async () => {
    ...
    it('shows a min error when there are 1 or 2 references', async () => {
    ...
    it('has no count error when there are exactly 3 references', async () => {
      ...
      await waitFor(() => {
        expect(getRadioError(container)).to.equal('');
      });
    });

    it('has no count error when there are exactly 4 references', async () => {
    ...
    it('shows a max error when there are more than 4 references', async () => {
      const five = [
        buildRef(0),
        buildRef(1),
        buildRef(2),
    ...
    it('still shows the required radio error when no selection is made', async () => {
