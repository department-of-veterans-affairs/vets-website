...
const validateAddressWithAPI = async (formData, errors, state, props) => {
  ...
  try {
    const validateAddressAction = await props.validateAddress(mailingAddress);

    // If we got suggested addresses and they're different from user input, show the validation modal
    if (
      validateAddressAction?.addresses?.length > 0 &&
      validateAddressAction.addresses[0].address !== mailingAddress
    ) {
      // Show the address validation modal
      props.setAddressValidationModalOpen(true);

      // Create a new error to prevent form submission
      // The user must select an address option first
      errors.addError('Please verify your address before continuing.');
    }
  } catch (error) {
