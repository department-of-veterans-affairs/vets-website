...
function transformForSubmit(formConfig, form) {
  ...
  // Helper function to format full name as string for signature
  const formatFullNameString = nameObj => {
    if (!nameObj) return '';
  ...
  const result = {
    ...
    recipient: {
      fullName: formatName(transformedData.recipientName),
      phone: {
        daytime: daytimePhone,
        evening: eveningPhone,
      },
      email: transformedData.recipient?.email || '',
      signature: formatFullNameString(transformedData.recipientName),
      signatureDate,
    },
    inReplyReferTo:
