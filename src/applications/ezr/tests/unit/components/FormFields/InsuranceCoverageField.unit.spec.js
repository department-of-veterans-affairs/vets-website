...
import InsuranceCoverageField from '../../../../components/FormFields/InsuranceCoverageField';
import content from '../../../../locales/en/content.json';

describe('ezr <InsuranceCoverageField>', () => {
  const getData = ({
    error = false,
    hasList = false,
    defaultValue = null,
  ...
  context('when providers list is empty', () => {
    it('should render with default question', () => {
      const { props } = getData({});
      const { container } = render(<InsuranceCoverageField {...props} />);
      const selector = container.querySelector('.schemaform-label');
      expect(selector).to.contain.text(content['insurance-coverage-question']);
    });
  });

  context('when providers list is not empty', () => {
