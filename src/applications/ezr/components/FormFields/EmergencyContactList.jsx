...
import { getEmergencyContactSrLabel } from '../../utils/helpers/emergencyContacts';
...
const EmergencyContactList = ({ labelledBy, list, mode, onDelete }) => {
  const scrollIdPrefix = 'ezr-contact-item';
  ...
  const listItems = veteranContacts.map((item, index) => {
    const { emergencyContactFullName, contactType } = item;
    const emergencyContactName = normalizeFullName(emergencyContactFullName);
    const srLabel = getEmergencyContactSrLabel(item);
    ...
    return (
      ...
      >
        <span
          ...
          data-dd-action-name="Veteran contact"
          aria-hidden
        >
          {emergencyContactName}
        </span>
        ...
        >
          {content['insurance-policy-tile-label']} {contactType}
        </span>
