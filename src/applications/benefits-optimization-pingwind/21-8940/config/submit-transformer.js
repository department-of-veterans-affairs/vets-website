...
const toVaNetworkFlag = value => {
  ...
  if (lower === 'nonva' || lower === 'non-va' || lower === 'non va' || lower === 'non_va') {
    return false;
  }
...
const toDateRangeArray = (items, startKey = 'startDate', endKey = 'endDate') => {
  ...
  pruneEmpty(
    toArray(doctors).map(entry =>
      pruneEmpty({
        ...
        relatedDisability: formatRelatedDisabilities(
          entry?.relatedDisability,
        ),
      }),
    ),
  );
