...
export default function AppealListItem({ appeal }) {
  ...
  const nextAction = appealStatusDescriptions(lastEvent).nextAction;
  ...
  return (
    <Link className="claim-list-item" to={`appeals/${appeal.id}/status`}>
      ...
      <p className="status"><span className="claim-item-label">Status:</span> {appealStatusDescriptions(lastEvent).status.title}</p>
