      ...
      <div className="usa-alert-body">
        ...
        {closedClaims.map(claim => (
          <p className="usa-alert-text claims-closed-text" key={claim.id}>
            <Link to={`your-claims/${claim.id}/status`}>Your {getClaimType(claim)} – Received {moment(claim.attributes.dateFiled).format('MMMM D, YYYY')}</Link> has been closed as of {moment(claim.attributes.phaseChangeDate).format('MMMM D, YYYY')}
          </p>
        ))}
      </div>
    </div>
