...
describe('Disability Benefits 526EZ <ConfirmationPage>', () => {
  it('should render', () => {
    ...
    expect(tree.find('.confirmation-page-title').text()).to.equal('Claim submitted');
    expect(tree.find('span').at(1).text().trim()).to.equal('For Sally  Alphonse');
    expect(tree.find('p').at(0).text()).to.contain('We usually process claims within 103 days.');
