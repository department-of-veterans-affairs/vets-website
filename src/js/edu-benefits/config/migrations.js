...
export function urlMigration(urlPrefix) {
  ...
  return (savedData) => {
    const savedReturnUrl = _.get('return_url', savedData.metadata).toLowerCase();

    if (savedReturnUrl.includes(urlPrefix)) {
      const newData = _.set('metadata.return_url', savedReturnUrl.replace(urlPrefix, ''), savedData);
      return newData;
    }
    return savedData;
  };
