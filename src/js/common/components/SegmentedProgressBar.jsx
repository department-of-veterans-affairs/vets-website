 ...
 */
export default function SegmentedProgressBar({ current, total, handleClick }) {
  return (
    <div className="progress-bar-segmented" role="progressbar" aria-valuenow={current} aria-valuemin="0" aria-valuemax={total} tabIndex="0">
      {_.range(total).map(step => (
        <div key={step} className={`progress-segment ${current > step ? 'progress-segment-complete' : ''}`} onClick={() => handleClick(step + 1)}/>
      ))}
    </div>
  );
}
