...
describe('Schemaform <FormStartControls>', () => {
  ...
  context('with React components', () => {
    ...
    it('should render start action link when logged in with no saved form', () => {
      ...
      const link = tree.baseElement.querySelector(
    ...
    it('should render continue & start new app buttons with a saved form', () => {
      ...
      buttons[1].click();
      expect(tree.baseElement.querySelector('va-modal[visible="true"]')).to
        .exist;
    });
  ...
  context('with va-link-action web component (only)', () => {
    beforeEach(() => {
      defaultRoutes = [
        'dummyProp',
        {
    ...
    it('should render start action link when logged in with no saved form', () => {
      ...
      const link = tree.baseElement.querySelector('va-link-action');
      expect(link).to.exist;
      expect(link.getAttribute('text')).to.contain('Get Started');
    });
    ...
    it('should render the action link', () => {
      ...
      const tree = render(
        <FormStartControls
          ...
          ariaLabel="test aria-label"
          ariaDescribedby="some description"
        />,
      );

      const formDOM = tree.baseElement.querySelector('va-link-action');
      expect(formDOM.getAttribute('text')).to.eq('Get Started');
      expect(formDOM.getAttribute('label')).to.eq('test aria-label');
      expect(formDOM.getAttribute('message-aria-describedby')).to.eq(
        'some description',
      );
    });
    ...
    it('should not throw a JS error when routes are missing', () => {
      ...
      expect(
        tree.baseElement.querySelectorAll('va-link-action').length,
      ).to.equal(1);
    });
  });
});
