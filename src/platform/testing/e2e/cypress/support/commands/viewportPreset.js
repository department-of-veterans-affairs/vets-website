...
const viewport390X780 = { width: 390, height: 780 };
...
const presets = {
  ...
  "iphone-12-mini": viewport390X780,
  // iPad presets
...
const isValidOrientation = orientation => orientation === 'portrait' || orientation === 'landscape';
const isValidOptions = options => {
  return typeof options === 'object' && options !== null &&
    options.log && options.log === true || options.log === false;
}
const presetError = "preset argument is invalid because preset doesn't exist";
const orientationError = "orientation argument must be 'portrait' or 'landscape'";
const optionsError = "options argument must be an object with a 'log' set to a boolean";

const setViewportHelper = (preset, orientation, options) => {
  if (isValidPreset(preset) && isValidOrientation(orientation) && isValidOptions(options)) {
    const { width, height } = presets[preset];
    orientation === 'portrait' ? cy.viewport(width, height, options) : cy.viewport(height, width, options);
    ...
    throw `Error: ${presetError}`;
  } else if (isValidPreset(preset) && isValidOptions(options)) {
    throw `Error: ${orientationError}`;
  } else if (isValidPreset(preset)) {
    throw `Error: ${orientationError} and ${optionsError}`;
  } else if (isValidOrientation(orientation)) {
    throw `Error: ${presetError} and ${optionsError}`;
  } else {
    throw `Error: ${presetError} and ${orientationError}`;
  }
}
...
*/
Cypress.Commands.add('viewportPreset', (preset, orientation = 'portrait', options = { log: true }) => {
  try {
