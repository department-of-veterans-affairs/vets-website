...
describe('arrayBuilderPages required parameters and props tests', () => {
  ...
  it('should throw error when required is a function and intro pages do not have depends', () => {
    try {
      arrayBuilderPages(
        {
          ...validOptionsRequiredFlow,
      ...
      expect(e.message).to.include(
        'When `arrayBuilderOptions` `required` is a function',
      );
      expect(e.message).to.include(
        'all `pageBuilder.introPage` pages must include a `depends` function',
      );
    }
  });
