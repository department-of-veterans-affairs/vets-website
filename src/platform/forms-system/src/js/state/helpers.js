...
export function recalculateSchemaAndData(initialState) {
  return Object.keys(initialState.pages).reduce((state, pageKey) => {
     ...
     * an issue in the schema recalculation is presented in the next if statements if two pages
     * (one active, one inactive) use the same data keys in their respective schemas. Thus we
     * should not need to recalculate inactive pages any further.
     */
