 ...
 * @property {(item: any) => string} [itemAriaLabel] Sets a custom aria label for each item in an array - for arrays. The item's index and the value at the itemAriaLabelDescriptorPath will follow this string in the aria label, if provided. Without this property, the itemName will be used. For example, with this property set to 'treatment facility', the edit aria label would read 'Edit treatment facility 1'.
 * @property {(item: any) => string} [itemAriaLabelDescriptorPath] Sets a custom descriptor for the aria label for each item in an array - for arrays. Without this property, the aria label will just use the item's index to distinguish it. For example, with this property, the edit aria label would read 'Edit treatment facility 1 Baptist Health' where itemAriaLabelDescriptorPath points to 'Baptist Health' in formData. Without, it would read 'Edit treatment facility 1'.
