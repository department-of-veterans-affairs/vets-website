...
function ObjectField({
  ...
  const SchemaField = pureWithDeepEquals(registry.fields.SchemaField);
  let orderedProperties = null;
  ...
  useEffect(
    () => {
      orderedProperties = orderAndFilterProperties(schema, uiSchema);
    },
    schema,
    uiSchema,
  );
  ...
  const onPropertyChange = name => {
    return value => {
      const formData = Object.keys(originalFormData || {}).length
        ? originalFormData
        : getDefaultFormState(schema, undefined, registry.definitions);
      onChange(_.set(name, value, formData));
    };
  };
