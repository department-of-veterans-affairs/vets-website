...
describe('<MHVApp>', () => {
  ...
  test('should show a loading indicator when fetching an account', () => {
  ...
  test(
    'should create an account if the user does not have an account but is eligible',
    () => {
  ...
  test('should redirect if the user needs to accepts T&C', () => {
    const wrapper = shallow(<MHVApp {...props}/>);
  ...
  test('should invoke upgrade if the user is only registered', () => {
    const wrapper = shallow(<MHVApp {...props}/>);
  ...
  test(
    'should invoke upgrade if the user is existing without access to the service',
    () => {
      const wrapper = shallow(<MHVApp {...props}/>);
  ...
  test(
    'should show a success message after the user accepts T&C and gets upgraded',
    () => {
  ...
  test(
    'should show MHV access error if nothing is loading or processing',
    () => {
      const wrapper = shallow(<MHVApp {...props}/>);
  ...
  test(
    'should show MHV access error if user has an account but not the required service',
    () => {
  ...
  test(
    'should render children if user has the required service as an existing user',
    () => {
      const newProps = merge(props, {
  ...
  test(
    'should render children if user has the required service as a registered user',
    () => {
      const newProps = merge(props, {
  ...
  test('should show placeholder account error', () => {
    const errors = [
      {
        title: 'Account error',
  ...
  test('should show error if unable to determine MHV account level', () => {
  ...
  test('should show error if failed to register MHV account', () => {
  ...
  test('should show error if failed to upgrade MHV account', () => {
    const newProps = set('mhvAccount.accountState', 'upgrade_failed', props);
    const wrapper = shallow(<MHVApp {...newProps}/>);
    expectAlert(wrapper, 'error', 'We can’t give you access to Vets.gov health tools right now');
  });

  test('should show error if the user has mismatched SSNs', () => {
  ...
  test('should show error if the user is not a VA patient', () => {
    const newProps = set('mhvAccount.accountState', 'needs_va_patient', props);
    const wrapper = shallow(<MHVApp {...newProps}/>);
    expectAlert(wrapper, 'error', 'We can’t give you access to the Vets.gov health tools');
  });

  test('should show error if the user has a disabled MHV account', () => {
  ...
  test('should show error if the user has multiple active MHV accounts', () => {
