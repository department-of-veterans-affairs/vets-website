...
const { createHealthCareStatusSuccess } = require('./endpoints/health-care');
const { v2 } = require('./endpoints/appointments');
const {
  createDebtsSuccess,
  createNoDebtsSuccess,
} = require('./endpoints/debts');
...
const responses = {
  ...
  'GET /v0/in_progress_forms/00-1234': (req, res) => {
    const secondsOfDelay = 1;
    delaySingleResponse(() => res.json(updateMemDb(req)), secondsOfDelay);
  },
  ...
  'PUT /v0/in_progress_forms/:id': (req, res) => {
    const secondsOfDelay = 1;
    updateInProgressForm(req.body, req.params.id);
    delaySingleResponse(
