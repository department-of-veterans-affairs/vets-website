...
describe('Feedback Reducer', () => {
  ...
  test('sets requestPending to true during action type SEND_FEEDBACK', () => {
    const result = reducer(undefined, { type: SEND_FEEDBACK, values: {} });
    expect(result.requestPending).to.be.true;
  });

  test(
    'sets requestPending to false during action type FEEDBACK_RECEIVED',
    () => {
  ...
  test(
    'sets the errorMessage to a string during action type FEEDBACK_ERROR anc clears it during CLEAR_FEEDBACK_ERROR',
    () => {
