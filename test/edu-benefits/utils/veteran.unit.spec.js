...
describe('veteranToApplication', () => {
  ...
  it('converts unset yes/no fields to undefined', () => {
    const formData = createVeteran();
    const applicationData = veteranToApplication(formData);

    expect(applicationData.serviceBefore1977.married).to.be.undefined;
  });
  ...
  it('removes seniorRotc when not applicable', () => {
    ...
    let applicationData = veteranToApplication(formData);

    expect(applicationData.seniorRotc).to.be.undefined;

    formData.seniorRotcCommissioned.value = 'Y';
    applicationData = veteranToApplication(formData);
