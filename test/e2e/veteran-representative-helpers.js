...
function completeClaimantInformation(client, data) {
  client
    ...
    .selectDropdown('root_claimantAddress_country', data.claimantAddress.country)
    .fill('input[name="root_claimantAddress_street"]', data.claimantAddress.street)
    .fill('input[name="root_claimantAddress_street2"]', data.claimantAddress.street2)
    ...
    .fill('input[name="root_claimantAddress_postalCode"]', data.claimantAddress.postalCode)
...
function completeVeteranServiceOrganizationInformation(client, data) {
  client
    ...
    .fill('input[name="root_organizationRepresentativeName"]', data.organizationRepresentativeName)
    .fill('input[name="root_organizationRepresentativeTitle"]', data.organizationRepresentativeTitle)
}

function completeAuthorizationForRepresentativeAccessToRecords(client, data) {
  client
    .click('input[type="checkbox"]')
}

function completeLimitationOfConsent(client, data) {
  client
    .click('input[name="root_disclosureExceptionDrugAbuse"]')
}

function completeAuthorizationToChangeClaimantAddress(client, data) {
  client
    .click('input[name="root_authorizationToChangeClaimantAddress"]')
}
