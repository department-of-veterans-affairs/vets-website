...
- ❌ **Never** access Redux state without `state.mr.` prefix
- ❌ **Never** skip error handling in async actions
- ❌ **Never** use v2 endpoints without checking feature flags
- ❌ **Never** forget to update `listState` during API calls
- ❌ **Never** display raw FHIR data without conversion functions
- ❌ **Never** use moment.js for new code - prefer date-fns
- ❌ **Never** skip Datadog error tracking in catch blocks
