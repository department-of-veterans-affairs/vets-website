...
### Inline Form Accordion Pattern
- **When to use**: Replace modals with inline forms for simple edit operations (per VADS guidance: "Modals should not contain long forms")
- **Implementation examples**: `CreateFolderInline.jsx`, `ManageFolderButtons.jsx`
- **Pattern**:
  ```jsx
  ...
  useEffect(() => {
    if (isExpanded && inputRef.current) {
      focusElement(inputRef.current.shadowRoot?.querySelector('input'));
    }
  }, [isExpanded]);

  // Pre-fill input when expanding (for edit operations)
  useEffect(() => {
    if (isExpanded && existingValue) {
      setInputValue(existingValue);
    }
  }, [isExpanded, existingValue]);

  const handleCancel = () => {
    setInputValue('');
    setIsExpanded(false);
    focusElement(triggerButtonRef.current); // Return focus to trigger
  };
  ...
  return (
    <div id="inline-form-id" data-testid="inline-form">
      <VaTextInput ref={inputRef} value={inputValue} onInput={...} />
...
- **Accessibility requirements**:
  - Trigger button must have `aria-expanded` and `aria-controls`
  - Focus moves to input when form expands
  - Focus returns to trigger button when cancelled
  - Use DataDog action tracking: `data-dd-action-name`
- **Anti-patterns**:
  - ❌ Don't use modal for simple single-field forms
  - ❌ Don't forget to pre-fill input value for edit operations
  - ❌ Don't leave focus on hidden elements after collapse

### Destructive Button Styling
- **When to use**: For actions that delete/remove content (e.g., "Remove folder", "Delete message")
- **Pattern**: Red outline button using inline styles (until VADS provides destructive variant)
  ```jsx
  <button
    type="button"
    className="usa-button-secondary vads-u-width--full"
    style={{ borderColor: '#b50909', color: '#b50909' }}
  ...
  ```
- **Note**: Deletion should still show confirmation modal - the red styling indicates severity, not skips confirmation
