...
3. Run `yarn watch --env entry=${entryName}`
If the user wants to run multiple entries, then check the manifest for each app, and run `yarn watch --env entry=${entryName1},${entryName2}`