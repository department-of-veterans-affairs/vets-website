function compareXml(doc1, doc2) {
  const nodeStack1 = [doc1.firstChild];
  const nodeStack2 = [doc2.firstChild];
  ...
  while (nodeStack1.length > 0 && nodeStack2.length > 0) {
    ...
    const node1 = nodeStack1.pop();
    let childNodes = node1.childNodes;
    for (let i = 0; i < childNodes.length; ++i) {
      nodeStack1.push(childNodes[i]);
    }

    const node2 = nodeStack2.pop();
    ...
    if (node1.nodeType !== node2.nodeType) {
      result.pass = false;
      result.message = `Expected ${expectNodeName(node1)} type ${node1.nodeType} to equal ${expectNodeName(node2)} type ${node2.nodeType}`;
      return result;
    }
    ...
    if (node1.nodeType === 1) {
      ...
      if (node1.namespaceURI !== node2.namespaceURI ||
        ...
        result.pass = false;
        result.message = `Expected ${expectNodeName(node1)} to have same qualified name as ${expectNodeName(node2)}`;
        return result;
      }

      // Compare the attributes.
      const attributeMap = {};
      let attributes = node1.attributes;
      let numNode1Attributes = 0;
      ...
      let numNode2Attributes = 0;
      for (let i = 0; i < attributes.length; ++i) {
        const attr = attributes[i];
        // Skip namespace attributes. They're not really attributes.
        if (attr.slice(0, 6) !== 'xmlns:') {
          ++numNode2Attributes;
          const mangledAttrName = `${attr.namespaceURI}<${attr.localName}`;
          if (attributeMap[mangledAttrName] !== attr.value) {
            result.pass = false;
            result.message = `Expected ${expectNodeName(node1)} attribute ${mangledAttrName} to be ${attr.value}`;
            return result;
          }
        }
      }
      if (numNode1Attributes !== numNode2Attributes) {
        result.pass = false;
        result.message = `Expected ${expectNodeName(node1)} to have same number of attributes as ${expectNodeName(node2)}`;
        return result;
      }
      ...
      if (node1.textContent !== node2.textContent) {
        result.pass = false;
        result.message = `Expected ${expectNodeName(node1)}: ${node1.textContent} to equal ${expectNodeName(node2)}: ${node2.textContent}`;
        return result;
      }
    }
  }
...
chai.use((_chai, utils) => {
  ...
  Assertion.addProperty('xml', function xmlProperty() {
      // flag it as xml
    flag(this, 'xml', true);
  });

  const compare = function xmlCompare(_super) {
    return function assertEqual(value) {
      if (flag(this, 'xml')) {
        const result = compareXml(this._obj, value);
        if (flag(this, 'negate')) {
...
describe('Health Care Form Feature Test', function healthFeatureTest() {
  this.timeout(60000);
  beforeEach((done) => {
  ...
  xit('Auto-saves on change events in the form.', () => {
    ...
    const changeEvent = contentWindow.document.createEvent('UIEvents');
  ...
  xit('should return true for valid monetary value input', () => {
    ...
    const validInput = ['55000.00', '100', '1.99', '10.'];
  ...
  xit('should return false for invalid monetary value input', () => {
    ...
    const form = HealthApp.forTest.getFormRoot();
    const testElement = form.elements['veteran[gross_wage_income]'];
    const invalidInput = ['1,000', 'abc', '$100', '10.0.0', '#$#', '\'\''];
