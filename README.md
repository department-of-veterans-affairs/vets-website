...
| run only unit tests for a subset of tests | `./node_modules/.bin/mocha path/to/my/test.unit.spec.jsx` use `--recursive 'path/to/my/**/*.unit.spec.js?(x)'` to test an entire directory (assumes current working dir of `/vets-website`) |
